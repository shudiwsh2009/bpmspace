...
public class DurationIndex implements CaseIndex {
	private static final String indexDirectory = "DurationIndex";
		...
		try {
			...
			for (Log log : logList) {
				if (log.getIndexFlag(0) == null) {
					try {
						for (int i = 0; i < log.getDurationList().size(); i++) {
							Document doc = DurationIndexDocument.Document(log,
									log.getDurationList().get(i), log.getId());

							indexWriter.addDocument(doc);
							indexWriter.commit();

						}
						log.setIndexFlagTrue(1);
						ma.saveLog(log);
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	public boolean destroy() {
		...
		try {
			...
			List<Log> logList = ma.findAllLogs();
			for (Log log : logList) {
				if (log.getIndexFlag(1) != null) {

					log.setIndexFlagFalse(1);
					ma.saveLog(log);
	...
	public void addInstance(Object o) {
		...
		if (log.getIndexFlag(1) == null) {
			try {
				for (int i = 0; i < log.getDurationList().size(); i++) {
					Document doc = DurationIndexDocument.Document(log, log
							.getDurationList().get(i), log.getId());
	...
	public List<LogQueryResult> getLogsLTE(Object o, int start, int end) {
		...
		try {
			...
			Query queryN = NumericRangeQuery.newIntRange(
					DurationIndexDocument.FIELDLENS, 1, end, true, true);

			DurationQueryResultCollector collector = new DurationQueryResultCollector(
					reader, start);
	...
	public List<LogQueryResult> getLogsGTE(Object o, int start, int end) {
		...
		try {
			...
			Query queryN = NumericRangeQuery.newIntRange(
					DurationIndexDocument.FIELDLENS, start, Integer.MAX_VALUE,
					true, true);
	...
	public List<LogQueryResult> getLogsBetween(Object o, int start, int end) {
		...
		try {
			...
			Query queryN = NumericRangeQuery.newIntRange(
					DurationIndexDocument.FIELDLENS, start, end, true, true);
	...
	public List<LogQueryResult> getLogs(Object o, int length) {
		...
		try {
			...
			Query querOne = NumericRangeQuery
					.newIntRange(DurationIndexDocument.FIELDLENS, length,
							length, true, true);
