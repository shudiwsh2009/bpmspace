...
public abstract class ColourDecorator implements Decorator {
	...
	public Paint getFillPaint(Paint paint, Point2D point1, Point2D point2) {
		if (fillColour1 != null && fillColour2 != null) {
			return new GradientPaint(point1, fillColour1, point2, fillColour2,
					false);
		} else {
	...
	public static boolean isDiscrete(String attributeName,
			Set<? extends Attributable> items) {
		...
		for (Object value : values) {
			if (value instanceof Number) {
				if (((Number) value).doubleValue() != ((Number) value)
						.intValue()) {
					isInteger = false;
				}
			} else {
	...
	public static boolean isNumerical(String attributeName,
			Set<? extends Attributable> items) {
		// First collect the set of all attribute values
		Set<Object> values = new HashSet<Object>();
		for (Attributable item : items) {
