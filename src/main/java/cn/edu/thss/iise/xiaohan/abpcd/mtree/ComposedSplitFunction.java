...
/**
 * A {@linkplain SplitFunction split function} that is defined by composing a
 * {@linkplain PromotionFunction promotion function} and a
...
public class ComposedSplitFunction<DATA> implements SplitFunction<DATA> {
	...
	@Override
	public SplitResult<DATA> process(Set<DATA> dataSet,
			DistanceFunction<? super DATA> distanceFunction) {
		Pair<DATA> promoted = promotionFunction.process(dataSet,
				distanceFunction);
		Pair<Set<DATA>> partitions = partitionFunction.process(promoted,
				dataSet, distanceFunction);
