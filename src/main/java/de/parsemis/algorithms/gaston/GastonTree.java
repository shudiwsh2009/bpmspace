...
public class GastonTree<NodeType, EdgeType> extends
	...
	public static <NodeType, EdgeType> GastonTree<NodeType, EdgeType> create(
		...
		nodes[ln] = lack = lend = new DepthRefinement(ln,
				path.getEdgeLabelIndex(le, env),
				path.getNodeLabelIndex(ln, env), rpmNodes);
		// create last refinement of right most node
		nodes[rn] = rack = rend = new DepthRefinement(rn,
				path.getEdgeLabelIndex(re, env),
				path.getNodeLabelIndex(rn, env), rpmNodes);
		...
		while (re != le) {
			...
			lack.prev = nodes[ln] = tmpr = new DepthRefinement(ln,
					path.getEdgeLabelIndex(le, env), path.getNodeLabelIndex(ln,
							env), rpmNodes);
			lack = tmpr;

			// create and step right
			rack.prev = nodes[rn] = tmpr = new DepthRefinement(rn,
					path.getEdgeLabelIndex(re, env), path.getNodeLabelIndex(rn,
							env), rpmNodes);
			rack = tmpr;
		...
		if (rn == ln) {
			rack.prev = nodes[rn] = tmpr = new DepthRefinement(rn,
					path.getEdgeLabelIndex(le, env), path.getNodeLabelIndex(rn,
							env), rpmNodes);
		...
		for (final Leg<NodeType, EdgeType> cur : siblings) {
			if (cur.ref.isCycleRefinement()) {
				...
				final DepthRefinement ref = new DepthRefinement(cdepth,
						cur.ref.getEdgeLabel(), cur.ref.getToLabel(), rpmNodes);
				if (cdepth != length
	 ...
	 * 
	 * @see
	 * de.parsemis.algorithms.gaston.GastonNode#extend(de.parsemis.algorithms
	 * .gaston.Leg, java.util.Collection)
	 */
	@Override
	public GastonNode<NodeType, EdgeType> extend(
		...
		if (ack.ref.isCycleRefinement()) {
			...
			rmpEbb.set(depth,
					rmpEbb.get(depth - 1) && bb[depth].compareTo(ackr) == 0);
			nright[depth++] = null;
	...
	public Collection<Extension<NodeType, EdgeType>> getExtensions() {
		...
		assert tenv.check(threadIdx) : "URGS! " + Thread.currentThread()
				+ " tenv:" + tenv.threadIdx + " shall:" + threadIdx;
