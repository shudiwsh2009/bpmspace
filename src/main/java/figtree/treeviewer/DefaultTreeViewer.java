...
public class DefaultTreeViewer extends TreeViewer {
	...
	public DefaultTreeViewer(JFrame frame) {
		...
		// This overrides MouseListener and MouseMotionListener to allow
		// selection in the TreePane -
	...
	public List<Tree> getTreesAsViewed() {
		...
		for (Tree originalTree : trees) {
			treeAsDisplayed.add(treePane
					.constructTransformedTree((RootedTree) originalTree));
		}
	...
	public void showTree(int index) {
		if (isRootingOn()
				&& getRootingType() == TreePane.RootingType.USER_ROOTING) {
			JOptionPane.showMessageDialog(frame,
					"Cannot switch trees when user rooting option is on.\n"
							+ "Turn this option off to switch trees",
					"Unable to switch trees", JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	public void setVerticalExpansion(double verticalExpansion) {
		double n = Math.max(treePane.getTree().getTaxa().size(), 50);
		this.verticalExpansion = Math.pow(verticalExpansion * n
				* VERTICAL_EXPANSION_SCALE, ZOOM_POWER);
	...
	public void selectTaxa(String attributeName, TextSearchType searchType,
	...
	public void selectNodes(String attributeName, TextSearchType searchType,
			String searchString, boolean caseSensitive) {
		treePane.clearSelection();
	...
	private boolean matchesItem(TextSearchType searchType, Object object,
			String query, boolean caseSensitive) {

		if (object != null) {
			String target = (caseSensitive ? object.toString() : object
	...
	public void selectTaxa(String attributeName, NumberSearchType searchType,
			Number searchValue) {
		treePane.clearSelection();
	...
	public void selectNodes(String attributeName, NumberSearchType searchType,
			Number searchValue) {
		treePane.clearSelection();
	...
	private boolean matchesItem(Object item, NumberSearchType searchType,
			Number searchValue) {
	...
	public void setSelectionMode(TreePaneSelector.SelectionMode selectionMode) {
		TreePaneSelector.SelectionMode oldSelectionMode = treePaneSelector
				.getSelectionMode();
	...
	// A load of delegated method calls through to treePane (which is now hidden
	// outside the package).
	...
	public void setBranchColouringDecorator(String branchColouringAttribute,
		...
		treePane.setBranchColouringDecorator(branchColouringAttribute,
