 ...
 */
public class ScaleAxisPainter extends LabelPainter<TreePane> implements
		ScalePainter {
	...
	public Rectangle2D calibrate(Graphics2D g2, TreePane treePane) {
		...
		preferredHeight = labelHeight + topMargin + bottomMargin
				+ scaleBarStroke.getLineWidth() + majorTickSize;

		if (!(treePane.getTreeLayout() instanceof RectilinearTreeLayout)) {
			// if the tree layout is not rectilinear, we are not going to
			// display
			// an axis at the moment so make the height 0.
			preferredHeight = 0.0;
		}

		tickLabelOffset = (float) (fm.getAscent() + topMargin + bottomMargin + majorTickSize)
				+ scaleBarStroke.getLineWidth();
	...
	public void paint(Graphics2D g2, TreePane treePane,
			Justification justification, Rectangle2D bounds) {
		Font oldFont = g2.getFont();
		...
		if (treePane.getTreeLayout() instanceof RadialTreeLayout) {
			// Unless the layout is the rectilinear or polar one, the grid won't
			// make sense...
			return;
		}
	...
	protected double getMaxTickLabelWidth(Graphics2D g2) {
		...
		if (axis.getLabelFirst()) { // Draw first minor tick as a major one
									// (with a label)
		...
		if (axis.getLabelLast()) { // Draw first minor tick as a major one (with
									// a label)
			label = axis.getFormatter()
					.format(axis.getMinorTickValue(0, n - 1));
			width = g2.getFontMetrics().stringWidth(label);
			if (maxWidth < width)
				maxWidth = width;
		}
	...
	protected void paintAxis(Graphics2D g2, Rectangle2D axisBounds) {
		...
		Line2D line = new Line2D.Double(minX, axisBounds.getMinY(), maxX,
				axisBounds.getMinY());
		g2.draw(line);
		...
		for (i = 0; i < n1; i++) {
			...
			if (i == (n1 - 1) && axis.getLabelLast()) { // Draw last minor tick
														// as a major one

				paintMajorTick(g2, axisBounds, axis,
						axis.getMinorTickValue(0, i));
	...
	protected void paintMajorTick(Graphics2D g2, Rectangle2D axisBounds,
			ScaleAxis axis, double value) {
		g2.setPaint(getForeground());
		g2.setStroke(getScaleBarStroke());
		...
		Line2D line = new Line2D.Double(pos, axisBounds.getMinY(), pos,
				axisBounds.getMinY() + majorTickSize);
		g2.draw(line);

		g2.setPaint(getForeground());
		double width = g2.getFontMetrics().stringWidth(label);
		g2.drawString(label, (float) (pos - (width / 2)),
				(float) (axisBounds.getMinY() + tickLabelOffset));
	}

	protected void paintMinorTick(Graphics2D g2, Rectangle2D axisBounds,
			double value) {
		...
		Line2D line = new Line2D.Double(pos, axisBounds.getMinY(), pos,
				axisBounds.getMinY() + minorTickSize);
		g2.draw(line);
	}
	...
	public void setDisplayAttribute(String displayAttribute) {
		throw new UnsupportedOperationException(
				"setDisplayAttribute not implemented in ScaleAxisPainter");
	}
	...
	private BasicStroke scaleBarStroke = new BasicStroke(1.0f,
