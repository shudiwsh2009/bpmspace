...
public class TreePane extends JComponent implements PainterListener, Printable {
	public enum RootingType {
		USER_ROOTING("User Selection"), MID_POINT("Midpoint");
	...
	public RootedTree constructTransformedTree(RootedTree sourceTree) {
		...
		if (isRootingOn) {
			...
			} else if (rootingType == RootingType.USER_ROOTING
					&& rootingNode != null) {
				Node left = newTree.getParent(rootingNode);
				if (left != null) {
					// rooting length should be [0, 1]

					double length = newTree.hasLengths() ? newTree
							.getLength(rootingNode) * rootingLength : 1.0;
					try {
						newTree = new ReRootedTree(newTree, left, rootingNode,
								length);
	...
	public void setCursorPosition(Point point) {
		...
		if (cursorPosition != null) {
			double xPos = (point.getX() - treeBounds.getX())
					/ treeBounds.getWidth();
			xPos = (xPos < 0.0 ? 0.0 : xPos > 1.0 ? 1.0 : xPos);
			double yPos = (point.getY() - treeBounds.getY())
					/ treeBounds.getHeight();
	...
	/**
	 * This returns the scaling factor between the graphical image and the
	...
	public void setRootAge(double rootAge) {
		double rootLength = timeScale.getHeight(rootAge, tree)
				- tree.getHeight(tree.getRootNode());
	...
	public void setTickSpacing(double userMajorTickSpacing,
			double userMinorTickSpacing) {
	...
	public void setBranchStroke(BasicStroke stroke) {
		...
		selectionStroke = new BasicStroke(
				Math.max(weight + 4.0F, weight * 1.5F), BasicStroke.CAP_ROUND,
	...
	public void setSelectionColor(Color selectionColor) {
		this.selectionPaint = new Color(selectionColor.getRed(),
	...
	public void setBranchTransform(
	...
	public void addSelectedNode(Node selectedNode, boolean toggle,
			boolean extend) {
	...
	public void addSelectedClade(Node selectedNode, boolean toggle,
			boolean extend) {
	...
	private void amendNodeSelection(Node selectedNode, boolean toggle,
			boolean extend) {
		if (!canSelectNode(selectedNode)) {
			return;
		}
	...
	private void amendCladeSelection(Node selectedNode, boolean toggle,
			boolean extend) {
		if (!canSelectNode(selectedNode)) {
			return;
		}
	...
	public void selectNodesFromSelectedTips() {
		if (selectedTips.size() > 0) {
			Node node = RootedTreeUtils.getCommonAncestorNode(tree,
					selectedTips);
			addSelectedClade(node, false, false);
		}
	...
	private void recalculateCollapsedNodes(Node node) {
		...
		if (!tree.isExternal(node)) {
			if (selectedNodes.contains(node)) {
				...
				Object[] oldValues = (Object[]) node
						.getAttribute(HILIGHT_ATTRIBUTE_NAME);
				if (oldValues != null) {
					int tipCount = RootedTreeUtils.getTipCount(tree, node);
					double height = RootedTreeUtils.getMinTipHeight(tree, node);
					Object[] values = new Object[] { tipCount, height,
	...
	/**
	 * Return whether the two axis scales should be maintained relative to each
	 * other
	 *
	 * @return a boolean
	 */
	...
	public void setPreferredSize(Dimension dimension) {
		if (treeLayout.maintainAspectRatio()) {
			super.setPreferredSize(new Dimension(dimension.width,
					dimension.height));
		} else {
	...
	public Node getNodeAt(Graphics2D g2, Point point) {
		...
		for (Node node : tree.getNodes()) {
			Shape branchPath = transform.createTransformedShape(treeLayoutCache
					.getBranchPath(node));
			if (branchPath != null && g2.hit(rect, branchPath, true)) {
				return node;
			}
			Shape collapsedShape = transform
					.createTransformedShape(treeLayoutCache
							.getCollapsedShape(node));
	...
	public RootedTree getSelectedSubtree() {
		...
		if (newTree.getRootNode() == null) {
			// no tree was constructed, most likely because only one tip was
			// selected
			return null;
		}
	...
	/**
	 * Returns a new node structure within newTree that contains the subtree
	 * subtended by selected nodes of the current tree.
	 * 
	 * @param newTree
	 * @param node
	 * @param isSelected
	 * @return
	 */
	private Node getSelectedSubtree(SimpleRootedTree newTree, Node node,
			boolean isSelected) {
		Node newNode;

		if (tree.isExternal(node)) {
			if (isSelected || selectedNodes.contains(node)
					|| selectedTips.contains(node)) {
			...
			if (children.size() == 0) {
				if (selectedNodes.contains(node)) {
					// if this node was selected but none of its children then
					// include the entire
					// descendent clade...
	...
	public void paint(Graphics graphics) {
		...
		// if (isCrosshairShown && cursorPosition != null && dragRectangle ==
		// null) {
		// g2.setPaint(cursorPaint);
		// g2.setStroke(cursorStroke);
		// double x = Math.max(treeBounds.getX(),
		// Math.min(cursorPosition.getX(), treeBounds.getX() +
		// treeBounds.getWidth()));
		// double y = Math.max(treeBounds.getY(),
		// Math.min(cursorPosition.getY(), treeBounds.getY() +
		// treeBounds.getHeight()));
		//
		...
		if (branchSelection == null) {
			...
			for (Node selectedNode : selectedNodes) {
				...
				if (branchPath != null) {
					Shape transPath = transform
							.createTransformedShape(branchPath);
					branchSelection.append(transPath, false);

				}
				Shape collapsedShape = treeLayoutCache
						.getCollapsedShape(selectedNode);
				if (collapsedShape != null) {
					Shape transPath = transform
							.createTransformedShape(collapsedShape);
					branchSelection.append(transPath, false);
				}
			}
		}
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex)
		...
		drawTree(g2, pageFormat.getImageableWidth(),
	...
	public void drawTree(Graphics2D g2, double width, double height) {
		...
		final boolean antialiasOn = rhints
		...
		// save graphics state which draw changes so that upon exit it can be
		// restored
		...
		if (legendPainter != null && legendPainter.isVisible()) {
			legendPainter.paint(g2, this, Painter.Justification.CENTER,
					legendBounds);
		}
		...
		for (ScalePainter scalePainter : scalePainters) {
			...
			if (scalePainter.isVisible()) {
				...
				scalePainter.paint(g2, this, Painter.Justification.CENTER,
						scaleBounds);
			}
		}
		...
		if (nodeBackgroundDecorator != null) {
			for (Node node : treeLayoutCache.getNodeAreaMap().keySet()) {
				...
				if (nodeArea != null) {
					...
					Shape transNodePath = transform
							.createTransformedShape(nodeArea);
					Paint background = new Color(0, 0, 0, 0);
					background = nodeBackgroundDecorator.getPaint(background);
					g2.setPaint(background);
					g2.fill(transNodePath);

					// Experimental outlining - requires order of drawing to be
					// pre-order
		...
		if (scaleGridPainter != null && scaleGridPainter.isVisible()) {
			Rectangle2D gridBounds = new Rectangle2D.Double(treeBounds.getX(),
					0.0, treeBounds.getWidth(), totalScaleBounds.getY());
		...
		for (Node node : treeLayoutCache.getBranchPathMap().keySet()) {
			...
			if (treeLayout.isShowingColouring()
					&& branchColouringAttribute != null) {
				branchColouring = (Object[]) node
						.getAttribute(branchColouringAttribute);
			}
			...
			if (branchColouring != null) {
				...
				for (int i = 0; i < branchColouring.length - 1; i += 2) {
					...
					g2.draw(new Line2D.Float(coords1[0], coords1[1],
							coords2[0], coords2[1]));

					coords1 = coords2;
				}

				// Draw the remaining branch as a path so it has proper line
				// joins...
				int colour = ((Number) branchColouring[branchColouring.length - 1])
						.intValue();
				branchColouringDecorator.setItem(colour);
				g2.setPaint(branchColouringDecorator.getPaint(Color.BLACK));
				...
				if (branchDecorator != null) {
					if (branchDecoratorGradient
							&& branchDecorator.allowsGradient()) {
						branchDecorator.setItems(node, tree.getParent(node));
						PathIterator iter = transPath.getPathIterator(null);
						double[] coords = new double[6];
						iter.currentSegment(coords);
						Point2D point1 = new Point2D.Double(coords[0],
								coords[1]);
						do {
							iter.currentSegment(coords);
							iter.next();
						} while (!iter.isDone());
						Point2D point2 = new Point2D.Double(coords[0],
								coords[1]);
		...
		// Paint node bars
		if (!isTransformBranchesOn() && nodeBarPainter != null
				&& nodeBarPainter.isVisible()) {
			for (Node node : nodeBars.keySet()) {
				Shape nodeBar = nodeBars.get(node);
				nodeBar = transform.createTransformedShape(nodeBar);
				nodeBarPainter.paint(g2, node,
						NodePainter.Justification.CENTER, nodeBar);
			}
		}
		...
		if (tipLabelPainter != null && tipLabelPainter.isVisible()) {
			...
			for (Node node : tipLabelTransforms.keySet()) {
				...
				AffineTransform tipLabelTransform = tipLabelTransforms
						.get(node);

				Painter.Justification tipLabelJustification = tipLabelJustifications
						.get(node);
				g2.transform(tipLabelTransform);

				tipLabelPainter.paint(g2, node, tipLabelJustification,
						new Rectangle2D.Double(0.0, 0.0, tipLabelWidth,
								tipLabelPainter.getPreferredHeight()));

				g2.setTransform(oldTransform);

				if (showingTipCallouts) {
					Shape calloutPath = transform
							.createTransformedShape(treeLayoutCache
									.getCalloutPath(node));
					if (calloutPath != null) {
		...
		if (nodeLabelPainter != null && nodeLabelPainter.isVisible()) {
			for (Node node : nodeLabelTransforms.keySet()) {
				...
				Painter.Justification nodeLabelJustification = nodeLabelJustifications
						.get(node);
				g2.transform(nodeTransform);

				nodeLabelPainter.paint(
						g2,
						node,
						nodeLabelJustification,
						new Rectangle2D.Double(0.0, 0.0, nodeLabelPainter
								.getPreferredWidth(), nodeLabelPainter
								.getPreferredHeight()));
		...
		if (branchLabelPainter != null && branchLabelPainter.isVisible()) {
			...
			for (Node node : branchLabelTransforms.keySet()) {
				...
				AffineTransform branchTransform = branchLabelTransforms
						.get(node);

				g2.transform(branchTransform);

				branchLabelPainter.calibrate(g2, node);
				final double preferredWidth = branchLabelPainter
						.getPreferredWidth();
				final double preferredHeight = branchLabelPainter
						.getPreferredHeight();

				// Line2D labelPath = treeLayout.getBranchLabelPath(node);

				branchLabelPainter.paint(g2, node,
						Painter.Justification.CENTER,
						// new Rectangle2D.Double(-preferredWidth/2,
						// -preferredHeight, preferredWidth, preferredHeight));
						new Rectangle2D.Double(0, 0, preferredWidth,
								preferredHeight));
	...
	private void calibrate(Graphics2D g2, double width, double height) {
		...
		// There are two sets of bounds here. The treeBounds are the bounds of
		// the elements
		// that make up the actual tree. These are scaled from branch length
		// space

		// The bounds are then the extra stuff that doesn't get scaled with the
		// tree such
		...
		for (Shape collapsedShape : treeLayoutCache.getCollapsedShapeMap()
				.values()) {
			// Add the bounds of the branch path to the overall bounds
		...
		// totalTreeBounds includes all the stuff which is not in a tree scale
		// (like labels and shapes) but in
		// screen pixel scale. This is added to the treeBounds to make space
		// round the edge.

		// add the tree bounds
		final Rectangle2D totalTreeBounds = treeBounds.getBounds2D(); // (YH)
																		// same
																		// as
																		// (Rectangle2D)
																		// treeBounds.clone();
		...
		if (tipLabelPainter != null && tipLabelPainter.isVisible()) {
			...
			// put this in a recursive function to allow for collapsed node
			// labels
			calibrateTipLabels(g2, tree.getRootNode(), tipLabelHeight,
					totalTreeBounds);
		}
		...
		if (nodeLabelPainter != null && nodeLabelPainter.isVisible()) {
			...
			for (Node node : treeLayoutCache.getNodeLabelPathMap().keySet()) {
				...
				final double labelHeight = nodeLabelPainter
						.getPreferredHeight();
				final double labelWidth = nodeLabelPainter.getPreferredWidth();
				Rectangle2D labelBounds = new Rectangle2D.Double(0.0, 0.0,
						labelWidth, labelHeight);

				// Work out how it is rotated and create a transform that
				// matches that
				AffineTransform labelTransform = calculateTransform(null,
						labelPath, labelWidth, labelHeight, true);

				// and add the translated bounds to the overall bounds
				totalTreeBounds.add(labelTransform.createTransformedShape(
		...
		if (branchLabelPainter != null && branchLabelPainter.isVisible()) {
			...
			for (Node node : treeLayoutCache.getBranchLabelPathMap().keySet()) {
				...
				final Line2D labelPath = treeLayoutCache
						.getBranchLabelPath(node);

				branchLabelPainter.calibrate(g2, node);
				final double labelHeight = branchLabelPainter.getHeightBound();
				final double labelWidth = branchLabelPainter
						.getPreferredWidth();
				...
				// matches that
				AffineTransform labelTransform = calculateTransform(null,
						labelPath, labelWidth, labelHeight, false);
		...
		if (nodeShapePainter != null && nodeShapePainter.isVisible()) {
			...
			for (Node node : tree.getNodes()) {
				...
				if (shapeBounds != null) {
					...
					// just at the centroid in here as the actual shape will be
					// reconstructed when drawing
					nodePoints.put(node,
							new Point2D.Double(shapeBounds.getCenterX(),
		...
		for (ScalePainter scalePainter : scalePainters) {
			if (scalePainter.isVisible()) {
				...
				Rectangle2D sb = new Rectangle2D.Double(treeBounds.getX(), y,
						treeBounds.getWidth(),
						scalePainter.getPreferredHeight());
				y += sb.getHeight();
		...
		if (legendPainter != null && legendPainter.isVisible()) {
			...
			legendBounds = new Rectangle2D.Double(-w2, 0, w2,
					treeBounds.getHeight());
		...
		if (maintainAspectRatio) {
			...
			double bottomDiff = (totalTreeBounds.getHeight() + totalTreeBounds
					.getY()) - (treeBounds.getHeight() + treeBounds.getY());
			double rightDiff = (totalTreeBounds.getWidth() + totalTreeBounds
					.getX()) - (treeBounds.getWidth() + treeBounds.getX());
			assert topDiff >= 0 && leftDiff >= 0 && bottomDiff >= 0
					&& rightDiff >= 0;
		...
		// small tree, long labels, label bounds may get larger that window,
		// protect against that

		if (xDiff >= availableW) {
			xDiff = Math.min(availableW, totalTreeBounds.getWidth())
					- treeBounds.getWidth();
		}

		if (yDiff >= availableH) {
			yDiff = Math.min(availableH, totalTreeBounds.getHeight())
					- treeBounds.getHeight();
		}
		...
		if (maintainAspectRatio) {
			...
			xOffset = ((width - (treeBounds.getWidth() * xScale)) / 2)
					- (treeBounds.getX() * xScale);
			yOffset = ((height - (treeBounds.getHeight() * yScale)) / 2)
					- (treeBounds.getY() * yScale);
		...
		for (Shape branchPath : treeLayoutCache.getBranchPathMap().values()) {
			// Add the bounds of the branch path to the overall bounds
			final Rectangle2D branchBounds = transform.createTransformedShape(
					branchPath).getBounds2D();
			if (treeBounds == null) {
				treeBounds = branchBounds;
			} else {
				treeBounds.add(branchBounds);
		...
		if (tipLabelPainter != null && tipLabelPainter.isVisible()) {
			...
			for (Node node : treeLayoutCache.getTipLabelPathMap().keySet()) {
				...
				// Work out how it is rotated and create a transform that
				// matches that
				AffineTransform taxonTransform = calculateTransform(transform,
						tipPath, tipLabelWidth, labelHeight, true);

				// Store the transformed bounds in the map for use when
				// selecting
				tipLabelBounds.put(node,
						taxonTransform.createTransformedShape(labelBounds));
				...
				final Painter.Justification just = (tipPath.getX1() < tipPath
						.getX2()) ? Painter.Justification.LEFT
		...
		if (nodeLabelPainter != null && nodeLabelPainter.isVisible()) {
			...
			final double labelWidth = nodeLabelPainter.getPreferredWidth();
			final Rectangle2D labelBounds = new Rectangle2D.Double(0.0, 0.0,
					labelWidth, labelHeight);
			...
			for (Node node : treeLayoutCache.getNodeLabelPathMap().keySet()) {
				...
				// Work out how it is rotated and create a transform that
				// matches that
				AffineTransform labelTransform = calculateTransform(transform,
						labelPath, labelWidth, labelHeight, true);

				// Store the transformed bounds in the map for use when
				// selecting
				nodeLabelBounds.put(node,
						labelTransform.createTransformedShape(labelBounds));
				...
				if (labelPath.getX1() < labelPath.getX2()) {
					nodeLabelJustifications.put(node,
							Painter.Justification.LEFT);
				} else {
					nodeLabelJustifications.put(node,
							Painter.Justification.RIGHT);
				}
		...
		if (branchLabelPainter != null && branchLabelPainter.isVisible()) {
			...
			for (Node node : treeLayoutCache.getBranchLabelPathMap().keySet()) {
				...
				final double labelHeight = branchLabelPainter
						.getPreferredHeight();
				final double labelWidth = branchLabelPainter
						.getPreferredWidth();
				final Rectangle2D labelBounds = new Rectangle2D.Double(0.0,
						0.0, labelWidth, labelHeight);

				final double dx = labelPath.getP2().getX()
						- labelPath.getP1().getX();
				final double dy = labelPath.getP2().getY()
						- labelPath.getP1().getY();
				final double branchLength = Math.sqrt(dx * dx + dy * dy);

				final Painter.Justification just = labelPath.getX1() < labelPath
						.getX2() ? Painter.Justification.LEFT
						: Painter.Justification.RIGHT;
				...
				AffineTransform labelTransform = calculateTransform(transform,
						labelPath, labelWidth, labelHeight, false);
				// move to middle of branch - since the move is before the
				// rotation
				final double direction = just == Painter.Justification.RIGHT ? 1
						: -1;
				labelTransform.translate(
						-direction * xScale * branchLength / 2, 0);

				// Store the transformed bounds in the map for use when
				// selecting
				branchLabelBounds.put(node,
						labelTransform.createTransformedShape(labelBounds));
		...
		if (nodeShapePainter != null && nodeShapePainter.isVisible()) {
			...
			for (Node node : nodePoints.keySet()) {
				...
				if (shapePath != null) {
					nodeShapeTransforms.put(node,
							calculateTransform(transform, shapePath));
				}
			}
		}
		...
		totalScaleBounds = new Rectangle2D.Double(treeBounds.getX(), y,
				treeBounds.getWidth(), 0.0);
		for (ScalePainter scalePainter : scalePainters) {
			if (scalePainter.isVisible()) {
				scalePainter.calibrate(g2, this);
				final double h1 = scalePainter.getPreferredHeight();
				Rectangle2D sb = new Rectangle2D.Double(treeBounds.getX(), y,
						treeBounds.getWidth(), h1);
				y += h1;
				totalScaleBounds.add(sb);
				scaleBounds.put(scalePainter, sb);
			}
		...
		if (legendPainter != null && legendPainter.isVisible()) {
			legendPainter.calibrate(g2, this);
			final double w2 = legendPainter.getPreferredWidth();
			legendBounds = new Rectangle2D.Double(0, 0, w2,
					treeBounds.getHeight());
		}
	...
	private void calibrateTipLabels(final Graphics2D g2, final Node node,
			final double tipLabelHeight, final Rectangle2D totalTreeBounds) {

		if (tree.isExternal(node)
				|| node.getAttribute(COLLAPSE_ATTRIBUTE_NAME) != null) {
			...
			Rectangle2D labelBounds = new Rectangle2D.Double(0.0, 0.0, width,
					tipLabelHeight);
			...
			if (taxonPath != null) {
				// Work out how it is rotated and create a transform that
				// matches that
				AffineTransform taxonTransform = calculateTransform(null,
						taxonPath, tipLabelWidth, tipLabelHeight, true);

				// and add the translated bounds to the overall bounds
				totalTreeBounds.add(taxonTransform.createTransformedShape(
						labelBounds).getBounds2D());
			}
		} else {
			for (Node child : tree.getChildren(node)) {
	...
	private AffineTransform calculateTransform(AffineTransform globalTransform,
			Line2D line, double width, double height, boolean just) {
		...
		// Now add a translate to the transform - if it is on the left then we
		// need
	...
	private ScaleAxis scaleAxis = new ScaleAxis(ScaleAxis.AT_DATA,
			ScaleAxis.AT_DATA);
	...
	private BasicStroke calloutStroke = new BasicStroke(0.5F,
			BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 1.0f, new float[] {
					0.5f, 2.0f }, 0.0f);
	private Stroke selectionStroke = new BasicStroke(6.0F,
			BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
	private Paint selectionPaint;
	private Stroke cursorStroke = new BasicStroke(0.5F, BasicStroke.CAP_BUTT,
