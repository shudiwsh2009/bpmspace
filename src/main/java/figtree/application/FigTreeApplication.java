...
public class FigTreeApplication extends MultiDocApplication {
	...
	public FigTreeApplication(MenuBarFactory menuBarFactory, String nameString,
			String aboutString, Icon icon, String websiteURLString,
			String helpURLString) {
		super(menuBarFactory, nameString, aboutString, icon, websiteURLString,
				helpURLString);
	...
	static public void createGraphic(String graphicFormat, int width,
			int height, String treeFileName, String graphicFileName) {
		...
		try {
			...
			ControlPalette controlPalette = new BasicControlPalette(
					FigTreePanel.CONTROL_PALETTE_WIDTH,
					BasicControlPalette.DisplayMode.ONLY_ONE_OPEN);
			FigTreePanel figTreePanel = new FigTreePanel(null, treeViewer,
					controlPalette);
	...
	public static void printTitle() {
		...
		centreLine("Thanks to Alexei Drummond, Joseph Heled, Philippe Lemey, ",
				60);
		centreLine(
	...
	public static void printUsage(Arguments arguments) {
		...
		arguments.printUsage("figtree",
				"[<tree-file-name>] [<graphic-file-name>]");
		System.out.println();
		System.out.println("  Example: figtree test.tree");
		System.out
				.println("  Example: figtree -graphic PDF test.tree test.pdf");
		System.out
				.println("  Example: figtree -graphic GIF -width 320 -height 320 test.tree test.gif");
		System.out.println();
	}
	...
	static public void main(String[] args) {
		...
		Arguments arguments = new Arguments(new Arguments.Option[] {
				new Arguments.StringOption("graphic", new String[] { "PDF",
						"SVG", "SWF", "PS", "EMF",
						// "PNG",
						"GIF",
				// "JPEG"
						}, false, "produce a graphic with the given format"),
				new Arguments.IntegerOption("width",
						"the width of the graphic in pixels"),
				new Arguments.IntegerOption("height",
		...
		if (arguments.hasOption("graphic")) {
			...
			if (args2.length == 0) {
				...
				createGraphic(graphicFormat, width, height, args2[0],
						(args2.length > 1 ? args2[1] : null));
				System.exit(0);
			} else {
				printTitle();
				createGraphic(graphicFormat, width, height, args2[0],
		...
		if (Utils.isMacOSX()) {
			...
			try {
				...
				javax.swing.SwingUtilities.invokeAndWait(new Runnable() {
					public void run() {
						try {
							// Only override the UI's necessary for ColorChooser
							// and
							// FileChooser:
							...
							try {
								...
								// this is to protect against the figtree.jar
								// being
		...
		java.net.URL url = FigTreeApplication.class
				.getResource("images/figtreeLogo.png");
		Icon icon = null;
		...
		String aboutString = "<html><center>Tree Figure Drawing Tool<br>Version "
				+ VERSION
				+ "<br>"
				+ DATES
				+ ", Andrew Rambaut<br>"
		...
		FigTreeApplication.application = new FigTreeApplication(
				new FigTreeMenuBarFactory(), nameString, aboutString, icon,
				websiteURLString, helpURLString);

		application.setDocumentFrameFactory(new DocumentFrameFactory() {
			public DocumentFrame createDocumentFrame(Application app,
		...
		// if (!jam.mac.Utils.isMacOSX() && application.getUpperDocumentFrame()
		// == null) {
