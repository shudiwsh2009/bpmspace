...
public class TimeScaleController extends AbstractController {
	...
	public TimeScaleController(final TreeViewer treeViewer) {
		...
		final boolean scaleRoot = PREFS.getBoolean(CONTROLLER_KEY + "."
				+ SCALE_ROOT_KEY, DEFAULT_SCALE_ROOT);
		final double offsetAge = PREFS.getDouble(CONTROLLER_KEY + "."
				+ OFFSET_AGE_KEY, DEFAULT_OFFSET_AGE);
		final double scaleFactor = PREFS.getDouble(CONTROLLER_KEY + "."
				+ SCALE_FACTOR_KEY, DEFAULT_SCALE_FACTOR);
		final double rootAge = PREFS.getDouble(CONTROLLER_KEY + "."
				+ ROOT_AGE_KEY, DEFAULT_ROOT_AGE);
		...
		final JLabel label1 = optionsPanel.addComponentWithLabel("Offset by:",
				offsetAgeText, true);

		scaleFactorText = new RealNumberField(-Double.MAX_VALUE,
				Double.MAX_VALUE);
		scaleFactorText.setValue(scaleFactor);
		final JLabel label2 = optionsPanel.addComponentWithLabel(
				"Scale factor:", scaleFactorText, true);
		...
		final JLabel label3 = optionsPanel.addComponentWithLabel("Root age:",
				rootAgeText, true);
	...
	public void setSettings(Map<String, Object> settings) {
		scaleFactorRadio.setSelected(!((Boolean) settings.get(CONTROLLER_KEY
				+ "." + SCALE_ROOT_KEY)));
		scaleRootRadio.setSelected(((Boolean) settings.get(CONTROLLER_KEY + "."
				+ SCALE_ROOT_KEY)));
		offsetAgeText.setValue((Double) settings.get(CONTROLLER_KEY + "."
				+ OFFSET_AGE_KEY));
		scaleFactorText.setValue((Double) settings.get(CONTROLLER_KEY + "."
				+ SCALE_FACTOR_KEY));
		rootAgeText.setValue((Double) settings.get(CONTROLLER_KEY + "."
				+ ROOT_AGE_KEY));
	}

	public void getSettings(Map<String, Object> settings) {
		settings.put(CONTROLLER_KEY + "." + SCALE_ROOT_KEY, new Boolean(
				scaleRootRadio.isSelected()));
		settings.put(CONTROLLER_KEY + "." + OFFSET_AGE_KEY,
				offsetAgeText.getValue());
		settings.put(CONTROLLER_KEY + "." + SCALE_FACTOR_KEY,
				scaleFactorText.getValue());
		settings.put(CONTROLLER_KEY + "." + ROOT_AGE_KEY,
				rootAgeText.getValue());
	}
