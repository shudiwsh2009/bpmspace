...
public class PolarTreeLayoutController extends AbstractController {
	...
	public PolarTreeLayoutController(final PolarTreeLayout treeLayout) {
		...
		rootAngleSlider
				.setValue((int) ((180.0 - (treeLayout.getRootAngle()) * 1000)));
		// rootAngleSlider.setMajorTickSpacing(rootAngleSlider.getMaximum() /
		// 5);
		...
		optionsPanel
				.addComponentWithLabel("Root Angle:", rootAngleSlider, true);

		angularRangeSlider = new JSlider(SwingConstants.HORIZONTAL, 0, 360000,
				0);
		angularRangeSlider.setOpaque(false);
		angularRangeSlider.setValue((int) ((360.0 - (treeLayout
				.getAngularRange()) * 1000.0)));
		// angularRangeSlider.setMajorTickSpacing(angularRangeSlider.getMaximum()
		// / 5);
		...
		optionsPanel.addComponentWithLabel("Angle Range:", angularRangeSlider,
				true);

		final int sliderMax = 10000;
		rootLengthSlider = new JSlider(SwingConstants.HORIZONTAL, 0, sliderMax,
				0);
		rootLengthSlider.setOpaque(false);
		rootLengthSlider
				.setValue((int) (treeLayout.getRootLengthProportion() * sliderMax));
		// rootLengthSlider.setMajorTickSpacing(rootLengthSlider.getMaximum() /
		// 5);
		...
		optionsPanel.addComponentWithLabel("Root Length:", rootLengthSlider,
				true);
		...
		// for (PolarTreeLayout.TipLabelPosition position :
		// PolarTreeLayout.TipLabelPosition.values()) {
		// if (position != PolarTreeLayout.TipLabelPosition.HORIZONTAL) // not
		// implemented yet
		...
		// treeLayout.setTipLabelPosition((PolarTreeLayout.TipLabelPosition)
		// labelPositionCombo.getSelectedItem());
		//
		// }
		// });
		// optionsPanel.addComponentWithLabel("Label Position:",
		// labelPositionCombo);
		...
		alignTipLabelsCheck
				.setSelected(treeLayout.getTipLabelPosition() == PolarTreeLayout.TipLabelPosition.RADIAL);
		alignTipLabelsCheck.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent changeEvent) {
				treeLayout.setTipLabelPosition(alignTipLabelsCheck.isSelected() ? PolarTreeLayout.TipLabelPosition.RADIAL
						: PolarTreeLayout.TipLabelPosition.FLUSH);
			}
		});
	...
	public void setSettings(Map<String, Object> settings) {
		rootAngleSlider.setValue((Integer) settings.get(POLAR_LAYOUT_KEY + "."
				+ ROOT_ANGLE_KEY));
		angularRangeSlider.setValue((Integer) settings.get(POLAR_LAYOUT_KEY
				+ "." + ANGULAR_RANGE_KEY));
		rootLengthSlider.setValue((Integer) settings.get(POLAR_LAYOUT_KEY + "."
				+ ROOT_LENGTH_KEY));
		showRootCheck.setSelected((Boolean) settings.get(POLAR_LAYOUT_KEY + "."
				+ SHOW_ROOT_KEY));
		alignTipLabelsCheck.setSelected((Boolean) settings.get(POLAR_LAYOUT_KEY
				+ "." + ALIGN_TIP_LABELS_KEY));
	}

	public void getSettings(Map<String, Object> settings) {
		settings.put(POLAR_LAYOUT_KEY + "." + ROOT_ANGLE_KEY,
				rootAngleSlider.getValue());
		settings.put(POLAR_LAYOUT_KEY + "." + ANGULAR_RANGE_KEY,
				angularRangeSlider.getValue());
		settings.put(POLAR_LAYOUT_KEY + "." + ROOT_LENGTH_KEY,
				rootLengthSlider.getValue());
		settings.put(POLAR_LAYOUT_KEY + "." + SHOW_ROOT_KEY,
				showRootCheck.isSelected());
		settings.put(POLAR_LAYOUT_KEY + "." + ALIGN_TIP_LABELS_KEY,
				alignTipLabelsCheck.isSelected());
	}
