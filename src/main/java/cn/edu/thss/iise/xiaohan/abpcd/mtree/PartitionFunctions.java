...
public final class PartitionFunctions {
	 ...
	 */
	public static class BalancedPartition<DATA> implements
		 ...
		 * </pre>
		 * 
		 * @see mtree.PartitionFunction#process(mtree.utils.Pair, java.util.Set,
		...
		public Pair<Set<DATA>> process(final Pair<DATA> promoted,
			...
			Collections.sort(queue1, new Comparator<DATA>() {
				...
				public int compare(DATA data1, DATA data2) {
					double distance1 = distanceFunction.calculate(data1,
							promoted.first);
					double distance2 = distanceFunction.calculate(data2,
							promoted.first);
			...
			Pair<Set<DATA>> partitions = new Pair<Set<DATA>>(
					new HashSet<DATA>(), new HashSet<DATA>());
