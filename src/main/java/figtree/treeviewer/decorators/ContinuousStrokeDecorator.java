...
public class ContinuousStrokeDecorator implements Decorator {
	...
	public Paint getPaint(final Paint paint, final Point2D point1,
			final Point2D point2) {
		throw new UnsupportedOperationException(
				"This decorator doesn't support gradients");
	}
	...
	public Paint getFillPaint(final Paint paint, final Point2D point1,
			final Point2D point2) {
		throw new UnsupportedOperationException(
				"This decorator doesn't support gradients");
	}
	...
	private void setAttributableItem(Attributable item) {
		...
		if (!Double.isNaN(value)) {
			stroke = new BasicStroke(
					(float) ((value * (maxWidth - minWidth)) + minWidth));
		} else {
			stroke = null;
		}
	}
