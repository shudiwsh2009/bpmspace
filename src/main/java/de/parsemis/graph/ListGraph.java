 ...
 * <p>
 * It stores the required node and edge informations in different lists
...
public class ListGraph<NodeType, EdgeType> implements
	...
	public Edge<NodeType, EdgeType> addEdge(
		...
		if (emptyEdge == -1) {
			idx = edges.size();
			final MyEdge ret = new MyEdge(this, idx, nodeA.getIndex(),
					nodeB.getIndex(), direction, label);
	 ...
	 * 
	 * @see
	 * src.de.pargra.graph.MutableGraph#addNodeAndEdge(src.de.pargra.graph.Node,
	...
	public Graph<NodeType, EdgeType> clone() {
		...
		for (final MyEdge edge : edges) {
			ret.addEdge(ret.getNode(edge.nodeA), ret.getNode(edge.nodeB),
