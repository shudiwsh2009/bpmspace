...
public class ScaleAxis {
	...
	protected double maxData = Double.NEGATIVE_INFINITY;// The number of major
	...
	protected int maxAxisFlag;// The distance between major ticks and minor
								// Ticks
	...
	protected double maxValue;// Flags to give automatic scaling and integer
								// division
	protected boolean isAutomatic = true;
	protected boolean isAutomaticTickSpacing = true;
	protected boolean isDiscrete = false;// Flags to specify that the first tick
											// and last tick should have labels.
	...
	protected DecimalFormat formatter = new DecimalFormat("0.0#######");// Used
																		// internally
	...
	public boolean getLabelLast() {
		if (getMinorTickCount(majorTickCount - 1) == 0) // The last tick is a
														// label anyway
			return false;
		else
			return labelLast;
	...
	/**
	 * Manually set the axis range. Axis flags must be set to AT_VALUE for this
	 * to take effect.
	 */
	 ...
	 * Manually set the axis ticks
	 */
	public void setManualAxis(double minTick, double maxTick, double majorTick,
			double minorTick) {
		...
		majorTickCount = (int) ((maxTick - minTick) / majorTick) + 1; // Add 1
																		// to
																		// include
																		// the
																		// last
																		// tick
		minorTickCount = (int) (majorTick / minorTick) - 1; // Sub 1 to exclude
															// the major tick
		isAutomatic = false;
		isAutomaticTickSpacing = false;
	...
	/**
	 * A static method that uses the natural log to obtain log to base10. This
	 * is required for the linear autoCalibrate but will also be used by a
	 * derived class giving a log transformed axis.
	 */
			...
			if (range < 1.0E-30) {
				if (minData < 0.0) {
					if (isAutomaticTickSpacing) {
						majorTick = Math.pow(10.0,
								Math.floor(log10(Math.abs(minData))));
					}
	...
	/**
	 * Calculate the optimum major tick distance. Override to change default
	 * behaviour
	 */
	public void calcMajorTick() {
		...
		while (majorTickCount < prefMajorTickCount) {
			...
			if (isDiscrete && (majorTick / 10) != Math.floor(majorTick / 10)) {
				// majorTick/10 is not an integer so no point in further
				// subdivision
				u = majorTick;
				majorTickCount = (int) (r / u);
				break;
	...
	/**
	 * Calculate the optimum minor tick distance. Override to change default
	 * behaviour
	 */
	public void calcMinorTick() {
		...
		while (minorTickCount < prefMinorTickCount) {
			...
			if (isDiscrete && (minorTick / 10) != Math.floor(minorTick / 10)) {
				// minorTick/10 is not an integer so no point in further
				// subdivision
				u = minorTick;
				minorTickCount = (int) (r / u);
				break;
	...
	public void handleAxisFlags() {
		...
		if (minAxisFlag == AT_MAJOR_TICK_PLUS
				|| minAxisFlag == AT_MINOR_TICK_PLUS) {
			if (minAxis == minData) {
				majorTickCount++;
				minTick -= majorTick;
				minAxis = minTick;
		...
		if (maxAxisFlag == AT_MAJOR_TICK_PLUS
				|| maxAxisFlag == AT_MINOR_TICK_PLUS) {
			if (maxAxis == maxData) {
				majorTickCount++;
				maxTick += majorTick;
				maxAxis = maxTick;
	...
	public double scaleValue(double value) {
		...
		double f = (transform(value) - transform(minAxis))
				/ (transform(maxAxis) - transform(minAxis));
		return f;
	}
	...
	/**
	 * Returns the number of minor tick marks within each major one By default
	 * all major ticks have the same number of minor ticks except the last which
	 * has none.
	 */
	...
	/**
	 * getMajorTick Returns the value of the majorTickIndex'th major tick
	 */
	...
	/**
	 * getMinorTick Returns the value of the minorTickIndex'th minor tick
	 */
	public double getMinorTickValue(int minorTickIndex, int majorTickIndex) {
		...
		if (majorTickIndex == -1)
			...
			return ((minorTickIndex + 1) * minorTick)
					+ getMajorTickValue(majorTickIndex);
	}
