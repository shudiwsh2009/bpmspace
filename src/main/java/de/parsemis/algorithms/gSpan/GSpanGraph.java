...
public class GSpanGraph<NodeType, EdgeType> implements
	...
	public void createInitials(
		...
		if (env.connectedFragments) {
			...
			for (int edge = availableEdges.nextSetBit(0); edge >= 0; edge = availableEdges
				...
				final GSpanEdge<NodeType, EdgeType> gedge = tenv
						.getEdge(0, (nodeA == nodeB ? 0 : 1),
								hp.getNodeLabelIndex(nodeA, env),
								hp.getEdgeLabelIndex(edge, env),
								hp.getNodeLabelIndex(nodeB, env),
								hp.getDirection(edge));
	 ...
	 * 
	 * @param edge
	 * @return <code>true</code>, if the given edge is frequent and not deleted
	 */
