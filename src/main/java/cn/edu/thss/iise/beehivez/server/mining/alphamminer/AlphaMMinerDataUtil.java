...
public class AlphaMMinerDataUtil {
	...
	public static AlphaSharpData convertRelation(AlphaPPData alphaPPData,
			LogEvents leEvents) {
		...
		result = new LogRelationsImpl(dmCausal, dmParallel, dmEnd, dmStart,
				dmOneLoop, leEvents);
	...
	public static void addRelation(AlphaPPData alphaPPData,
		...
		for (int i = 0; i < size; i++) {
			...
			String taskName = logEvent.getModelElementName() + "\0"
		...
		for (int i = 0; i < size; i++)
			for (int j = 0; j < size; j++) {
				...
				if (dmCausal.get(i, j) > 0) {
					...
					_rmRelation.addRelation(taskNameI, taskNameJ,
							Relation.BEFORE);
		...
		for (int i = 0; i < size; i++)
			for (int j = 0; j < size; j++) {
				...
				if (dmParal.get(i, j) > 0) {
					...
					_rmRelation.addRelation(taskNameJ, taskNameI,
							Relation.BEFORE);
		...
		for (int i = 0; i < size; i++)
			for (int j = 0; j < size; j++) {
				...
				if (dmCausal.get(i, j) > 0) {
					String taskNameI = _alT_log.get(i);
					String taskNameJ = _alT_log.get(j);
					TaskUnit tuI = (TaskUnit) _rmRelation.htRelations
							.get(taskNameI);
		...
		for (int i = 0; i < size; i++)
			for (int j = 0; j < size; j++) {
				...
				if (dmParal.get(i, j) > 0) {
					...
					tuI.removePred(taskNameJ); // TODO
												// 没搞明白为啥不需要pred里面.因为不是before关系了哦！但是succ保留下了，因为succ关系当且仅当->才可以
					tuI.addPara(taskNameJ);
					tuI.addRela(taskNameJ, Relation.PARALLEL);
				}
			}
		// 10.aba,
		// 没有考虑，因为aba的作用是寻找->和||,但是->和||已经在alpha#中找出来了，所以不需要这个关系了，呵呵,实际上在alpha#中也没法考虑这个关系啊
		// 需要考虑，因为alphasharp.invisible_1_49等5个模型则算不出来。aba,bab的关系时a->b,b->a,a\||b
		for (int i = 0; i < size; i++)
			for (int j = 0; j < size; j++) {
				if (i == j)
					continue;
				if (dmCausal.get(i, j) > 0 && dmCausal.get(j, i) > 0
						&& dmParal.get(i, j) == 0) {
					String taskNameI = _alT_log.get(i);
					String taskNameJ = _alT_log.get(j);
					_rmRelation.addRelation(taskNameI, taskNameJ,
	...
	public static boolean belongs(HashSet<Integer> a, HashSet<Integer> b) {
		for (Integer i : a)
			if (!b.contains(i))
				return false;
		return true;
	...
	public static HashSet<Integer> removeParallel(HashSet<Integer> betweenYX,
		...
		for (Integer i : betweenYX) {
			if (relations.getParallelMatrix().get(y, i) == 1
					|| relations.getParallelMatrix().get(i, x) == 1)
				continue;
			result.add(i);
		}
		return result;
	...
	public static <T> ArrayList<T> union(ArrayList<T> x, ArrayList<T> y) {
		ArrayList<T> result = new ArrayList<T>();
		for (T t : x)
			result.add(t);
		for (T t : y)
