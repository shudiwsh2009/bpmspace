...
public class Arguments {
	...
	public static class StringOption extends Option {
		...
		public StringOption(String label, String[] options,
				boolean caseSensitive, String description) {
			super(label, description);
	...
	public static class IntegerOption extends Option {
		...
		public IntegerOption(String label, int minValue, int maxValue,
				String description) {
			super(label, description);
	...
	public static class IntegerArrayOption extends IntegerOption {
		...
		public IntegerArrayOption(String label, int count, String description) {
			this(label, count, Integer.MIN_VALUE, Integer.MAX_VALUE,
					description);
		}

		public IntegerArrayOption(String label, int minValue, int maxValue,
				String description) {
			this(label, 0, minValue, maxValue, description);
		}

		public IntegerArrayOption(String label, int count, int minValue,
				int maxValue, String description) {
	...
	public static class RealOption extends Option {
		...
		public RealOption(String label, double minValue, double maxValue,
				String description) {
			super(label, description);
			this.minValue = minValue;
			this.maxValue = maxValue;
	...
	public static class RealArrayOption extends RealOption {
		...
		public RealArrayOption(String label, String description) {
			this(label, 0, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY,
					description);
		}

		public RealArrayOption(String label, int count, String description) {
			this(label, count, Double.NEGATIVE_INFINITY,
					Double.POSITIVE_INFINITY, description);
		}

		public RealArrayOption(String label, double minValue, double maxValue,
				String description) {
			this(label, 0, minValue, maxValue, description);
		}

		public RealArrayOption(String label, int count, double minValue,
				double maxValue, String description) {
			super(label, minValue, maxValue, description);
			this.count = count;
		}
	...
	public void parseArguments(String[] arguments) throws ArgumentException {
			...
			if (index != -1) {
				...
				if (optionIndex[index] != -1) {
					throw new ArgumentException("Argument, " + arguments[index]
							+ " overlaps with another argument");
				}

				// the first value may be appended to the option label (e.g.,
				// '-t1.0'):
				String arg = arguments[index]
						.substring(option.label.length() + 1);
				optionIndex[index] = i;
				...
				if (option instanceof IntegerArrayOption) {
								...
								if (token.length() > 0) {
									try {
										...
										throw new ArgumentException(
												"Argument, "
														+ arguments[index]
														+ " has a bad integer value: "
														+ token);
									}
									if (o.values[j] > o.maxValue
											|| o.values[j] < o.minValue) {
										throw new ArgumentException(
												"Argument, "
														+ arguments[index]
														+ " has a bad integer value: "
						...
						if (j < o.count) {
							if (k >= arguments.length) {
								throw new ArgumentException(
										"Argument, "
												+ arguments[index]
												+ " is missing one or more values: expecting "
												+ o.count + " integers");
							}
					...
					try {
						...
						throw new ArgumentException("Argument, "
								+ arguments[index] + " has a bad real value: "
								+ arg);
					}
					if (o.value > o.maxValue || o.value < o.minValue) {
						throw new ArgumentException("Argument, "
		...
		while (i >= 0 && optionIndex[i] == -1
				&& !arguments[i].startsWith(ARGUMENT_CHARACTER)) {
			n++;
			i--;
		}
		...
		for (i = 0; i < arguments.length - n; i++) {
			if (optionIndex[i] == -1) {
				throw new ArgumentException("Unrecognized argument: "
						+ arguments[i]);
			}
		}
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (arguments[i].length() - 1 >= label.length()) {
				if (arguments[i].startsWith(ARGUMENT_CHARACTER)) {
					...
					if ((!caseSensitive && label.equalsIgnoreCase(l))
							|| label.equals(l)) {
						return i;
					}
