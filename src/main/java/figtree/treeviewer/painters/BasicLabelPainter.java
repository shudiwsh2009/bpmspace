...
/**
 * A simple implementation of LabelPainter that can be used to display tip, node
 * or branch labels. It can display, taxon names, branch lengths, node heights
 * or other attributeNames of nodes.
 *
...
public class BasicLabelPainter extends LabelPainter<Node> {
	...
	public void paint(Graphics2D g2, Node item, Justification justification,
			Rectangle2D bounds) {
		Tree tree = treePane.getTree();
		...
		if (textDecorator != null) {
			// this is a bit of a hack to detect whether we should be getting
			// the
		...
		if (label != null) {
			...
			case CENTER:
				...
				// xOffset = (float) (bounds.getX() + (bounds.getWidth() -
				// rect.getWidth()) / 2.0);
				break;
			case FLUSH:
			case LEFT:
