...
public class HSBContinuousColourDecorator extends ContinuousColourDecorator {
	...
	public HSBContinuousColourDecorator(ContinuousScale continuousScale)
	...
	public void setup(String settings) {
		if (!settings.startsWith("{") || !settings.endsWith("}")) {
			throw new IllegalArgumentException(
					"HSBContinuousColourDecorator settings string not in correct format");
		}
	...
	public void setup(float hueUpper, float hueLower, float saturationUpper,
			float saturationLower, float brightnessUpper,
			float brightnessLower, boolean reverseHue) {
	...
	protected Color getColourForScaledValue(double value) {
		if (!Double.isNaN(value)) {
			return Color.getHSBColor(getHue((float) value),
					getSaturation((float) value), getBrightness((float) value));
		} else {
			return null;
		}
	}
