...
public class DiscreteColourScaleDialog {
	...
	private JComboBox colourSchemeCombo = new JComboBox(
			new String[] { HSB_SPECTRUM /* , FIXED_COLOURS */});
	...
	public DiscreteColourScaleDialog(final JFrame frame) {
		...
		// colourSchemeNamePanelMap.put(FIXED_COLOURS, new
		// FixedColourSchemePanel());

		colourSchemeClassNameMap.put(HSBDiscreteColourDecorator.class,
				HSB_SPECTRUM);
		// colourSchemeClassNameMap.put(FixedDiscreteColourDecorator.class,
		// FIXED_COLOURS);
	...
	public int showDialog() {
		...
		// options.addSpanningComponent(new
		// JLabel("Editing colour range for attribute: " +
		// decorator.getAttributeName()));
	...
	interface Reorderable {
		public void reorder(java.util.List<Integer> sourceIndices,
	...
	class ColourTableModel extends DefaultTableModel implements Reorderable {
		...
		public Object getValueAt(int row, int column) {
			...
			case 1:
				// until the OK button is pressed the trait values are not
				// actually
	...
	public class TableRowTransferHandler extends TransferHandler {
		// private final DataFlavor localObjectFlavor = new
		// ActivationDataFlavor(Integer.class,
		// DataFlavor.javaJVMLocalObjectMimeType, "Integer Row Index");
		private final DataFlavor localObjectFlavor = new DataFlavor(
				ArrayList.class, "Integer Row Index");
		...
		protected Transferable createTransferable(JComponent c) {
			...
			return new DataHandler(selectedRows,
		...
		public boolean importData(TransferHandler.TransferSupport info) {
			...
			try {
				java.util.List<Integer> selectedRows = (java.util.List) info
						.getTransferable().getTransferData(localObjectFlavor);
				if (selectedRows != null && selectedRows.size() > 0) {
					((Reorderable) table.getModel()).reorder(selectedRows,
							index);
	...
	private class HSBColourSchemePanel implements ColourSchemePanel {
		...
		public void setDecorator(DiscreteColourDecorator decorator) {
			if (decorator instanceof HSBDiscreteColourDecorator) {
				...
				if (hsbDecorator == null) {
					hsbDecorator = new HSBDiscreteColourDecorator(
							decorator.getAttributeName());
				}
			}
		...
		public JPanel getPanel() {
			if (panel == null) {
				...
				options.addComponentWithLabel("Secondary count: ",
						secondaryCountSpinner);
		...
		private JComboBox primaryAxisCombo = new JComboBox(
				HSBDiscreteColourDecorator.Axis.values());
		private SpinnerNumberModel secondaryCountSpinnerModel = new SpinnerNumberModel(
				2, 1, 100, 1);
		private JSpinner secondaryCountSpinner = new JSpinner(
		...
		public void setDecorator(DiscreteColourDecorator decorator) {
			if (decorator instanceof FixedDiscreteColourDecorator) {
				...
				if (fixedDecorator == null) {
					fixedDecorator = new FixedDiscreteColourDecorator(
							decorator.getAttributeName());
				}
			}
