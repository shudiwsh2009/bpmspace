...
public class FigTreeApplet extends JApplet {
	...
	public void init() {
		...
		figTreePanel = new FigTreePanel(treeViewer, controlPalette1,
				controlPalette2, style);
		...
		filterPanel.addSearchPanelListener(new SearchPanelListener() {
			...
			public void searchStarted(String searchString) {
				...
				if (index == -1)
					index = 0;
				TreeViewer.TextSearchType searchType = TreeViewer.TextSearchType
						.values()[index];
	...
	protected boolean readData(Reader reader, boolean isNexus)
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Error reading tree file: "
					+ ie, "Import Error", JOptionPane.ERROR_MESSAGE);
			return false;
		}
