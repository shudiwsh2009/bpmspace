...
public class FigTreeMacFileMenuFactory implements MenuFactory {
	...
	public void populateMenu(JMenu menu, AbstractFrame frame) {
		...
		item.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S,
				MenuBarFactory.MENU_MASK + ActionEvent.SHIFT_MASK));
		menu.add(item);
		...
		// On Windows and Linux platforms, each window has its own menu so items
		// which are not needed
		// are simply missing. In contrast, on Mac, the menu is for the
		// application so items should
		...
		if (frame instanceof FigTreeFileMenuHandler) {
			...
			item = new JMenuItem(
					((FigTreeFileMenuHandler) frame)
							.getImportColourSchemeAction());
			menu.add(item);

			menu.addSeparator();

			item = new JMenuItem(
					((FigTreeFileMenuHandler) frame).getExportTreesAction());
			item.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E,
					MenuBarFactory.MENU_MASK));
			menu.add(item);

			item = new JMenuItem(
					((FigTreeFileMenuHandler) frame).getExportGraphicAction());
			item.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E,
					MenuBarFactory.MENU_MASK + KeyEvent.ALT_MASK));
			menu.add(item);

			item = new JMenuItem(
					((FigTreeFileMenuHandler) frame).getExportPDFAction());
			item.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E,
					MenuBarFactory.MENU_MASK + KeyEvent.SHIFT_MASK
							+ KeyEvent.ALT_MASK));
			menu.add(item);

			/*
			 * to be added... menu.addSeparator();
			 * 
			 * item = new
			 * JMenuItem(((FigTreeFileMenuHandler)frame).getImportColourSchemeAction
			 * ()); menu.add(item);
			 * 
			 * item = new
			 * JMenuItem(((FigTreeFileMenuHandler)frame).getExportColourSchemeAction
			 * ()); menu.add(item);
			 */
		} else {
			// If the frame is not a TracerFileMenuHandler then create a dummy
			// set of disabled menu options.
			// At present the only situation where this may happen is in Mac OS
			// X when no windows
