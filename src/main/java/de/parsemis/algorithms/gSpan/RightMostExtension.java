			...
			for (int i = sup.getDegree(nodeA) - 1; i >= 0; --i) {
				...
				if (emb.freeSuperEdge(edge)) {
					...
					final GSpanEdge<NodeType, EdgeType> gEdge = tenv.getEdge(
							...
							sup.getEdgeLabelIndex(edge, env),
							sup.getNodeLabelIndex(oNode, env),
							sup.getDirection(edge, nodeA));
			...
			do {
				...
				for (int i = sup.getDegree(nodeA) - 1; i >= 0; --i) {
					...
					if (emb.freeSuperEdge(edge)
						...
						add(tenv.getEdge(ackNode, lastNode + 1,
								sup.getNodeLabelIndex(nodeA, env),
								sup.getEdgeLabelIndex(edge, env),
								sup.getNodeLabelIndex(oNode, env),
								sup.getDirection(edge, nodeA)), emb, code,
								edge, oNode);
					}
				}
			} while (ackNode > 0);
