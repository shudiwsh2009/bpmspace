...
public class PolarTreeLayout extends AbstractTreeLayout {
	...
	public double getHeightOfPoint(Point2D point) {
		throw new UnsupportedOperationException(
				"Method getHeightOfPoint() is not supported in this TreeLayout");
	}
	...
	public Shape getHeightArea(double height1, double height2) {
		Area area1 = new Area(new Ellipse2D.Double(0.0, 0.0, height2 * 2.0,
				height2 * 2.0));
		Area area2 = new Area(new Ellipse2D.Double(0.0, 0.0, height1 * 2.0,
				height1 * 2.0));
	...
	public void layout(RootedTree tree, TreeLayoutCache cache) {
		...
		final Point2D rootPoint = constructNode(tree, root, 0.0,
				totalRootLength, /* new Area(), */cache);
		...
		if (showingRootBranch) {
			...
			Line2D line = new Line2D.Double(transform(0.0, y), transform(
					rootPoint.getX(), y));
	...
	private Point2D constructNode(RootedTree tree, Node node, double xParent,
			double xPosition, /* final Area parentNodeArea, */
		...
		if (!tree.isExternal(node)) {
			...
			if (collapsedAttributeName != null
					&& node.getAttribute(collapsedAttributeName) != null) {
				nodePoint = constructCollapsedNode(tree, node, xPosition, cache);
			} else if (cartoonAttributeName != null
					&& node.getAttribute(cartoonAttributeName) != null) {
				...
				for (int i = 0; i < childList.size(); i++) {
					...
					childPoints[i] = constructNode(tree, children[i],
							xPosition, xPosition + length, cache);
				...
				for (int i = 0; i < childList.size(); i++) {
					...
					if (branchColouringAttribute != null) {
						colouring = (Object[]) children[i]
								.getAttribute(branchColouringAttribute);
					}
					if (colouring != null) {
						// If there is a colouring, then we break the path up
						// into
						...
						branchPath.moveTo((float) transformedChildPoint.getX(),
								(float) transformedChildPoint.getY());

						float x = (float) x1;
						for (int j = 0; j < colouring.length - 1; j += 2) {
							// double height =
							// ((Number)colouring[j+1]).doubleValue();
							// double p = (height - childHeight) / (nodeHeight -
							// childHeight);
							float interval = ((Number) colouring[j + 1])
									.floatValue();
							float p = interval / (nodeHeight - childHeight);
							x -= ((x1 - x0) * p);
							final Point2D transformedPoint = transform(x,
									childPoints[index].getY());
							branchPath.lineTo((float) transformedPoint.getX(),
					...
					arc.setArcByCenter(0.0, 0.0, nodePoint.getX(), finish,
							start - finish, Arc2D.OPEN);
					branchPath.append(arc, true);

					// if (i == childList.size() - 1) {
					// Arc2D arc2 = new Arc2D.Double();
					// arc2.setArcByCenter(0.0, 0.0, maxXPosition,
					// firstChildAngle, finish - firstChildAngle, Arc2D.OPEN);
					...
					// arc3.setArcByCenter(0.0, 0.0, nodePoint.getX(), finish,
					// firstChildAngle - finish, Arc2D.OPEN);
					...
					final double x3 = (nodePoint.getX() + childPoints[index]
							.getX()) / 2;

					Line2D branchLabelPath = new Line2D.Double(transform(
							x3 - 1.0, childPoints[index].getY()), transform(
							x3 + 1.0, childPoints[index].getY()));
				...
				Line2D nodeLabelPath = new Line2D.Double(transform(
						nodePoint.getX(), yPos), transform(
						nodePoint.getX() + 1.0, yPos));
				cache.nodeLabelPaths.put(node, nodeLabelPath);

				Line2D nodeShapePath = new Line2D.Double(transform(
						nodePoint.getX(), yPos), transform(
						nodePoint.getX() - 1.0, yPos));
			...
			if (tipLabelPosition == TipLabelPosition.FLUSH) {
				...
				tipLabelPath = new Line2D.Double(transformedNodePoint,
						transform(xPosition + 1.0, yPosition));

			} else if (tipLabelPosition == TipLabelPosition.RADIAL) {

				tipLabelPath = new Line2D.Double(transform(maxXPosition,
						yPosition), transform(maxXPosition + 1.0, yPosition));

				Line2D calloutPath = new Line2D.Double(transformedNodePoint,
						transform(maxXPosition, yPosition));
			...
			Line2D nodeShapePath = new Line2D.Double(transform(
					nodePoint.getX(), yPosition), transform(
					nodePoint.getX() - 1.0, yPosition));
			cache.nodeShapePaths.put(node, nodeShapePath);
	...
	private void constructNodeAreas(final RootedTree tree, final Node node,
			final Area parentNodeArea, TreeLayoutCache cache) {
		...
		if (!tree.isExternal(node)) {
			...
			PathIterator pi1 = cache.getBranchPath(child1)
					.getPathIterator(null);
			nodePath.append(pi1, false);

			// start point
			// final float x0 = (float) branchBounds1.getX();
			// final float y0 = (float) (branchBounds1.getY() +
			// branchBounds1.getHeight());
			...
			//
			// final float y2 = (float) (branchBounds2.getY() +
			// branchBounds2.getHeight());
	...
	private Point2D constructCartoonNode(RootedTree tree, Node node,
			double xPosition, TreeLayoutCache cache) {
		...
		Point2D transformedNodePoint1 = transform(new Point2D.Double(maxXPos,
				minYPos));
		Point2D transformedNodePoint2 = transform(new Point2D.Double(maxXPos,
				maxYPos));

		GeneralPath collapsedShape = new GeneralPath();

		collapsedShape.moveTo((float) transformedNodePoint0.getX(),
				(float) transformedNodePoint0.getY());
		collapsedShape.lineTo((float) transformedNodePoint1.getX(),
				(float) transformedNodePoint1.getY());
		final double start = getAngle(maxYPos);
		final double finish = getAngle(minYPos);
		Arc2D arc = new Arc2D.Double();
		arc.setArcByCenter(0.0, 0.0, maxXPos, finish, start - finish,
				Arc2D.OPEN);
		...
		Line2D nodeLabelPath = new Line2D.Double(transform(nodePoint.getX(),
				yPos), transform(nodePoint.getX() + 1.0, yPos));

		cache.nodeLabelPaths.put(node, nodeLabelPath);

		Line2D nodeBarPath = new Line2D.Double(
				transform(nodePoint.getX(), yPos), transform(
						nodePoint.getX() - 1.0, yPos));

		cache.nodeShapePaths.put(node, nodeBarPath);

		if (showingCartoonTipLabels) {
			constructCartoonTipLabelPaths(tree, node, maxXPos,
					new double[] { minYPos }, cache);
		}
	...
	private void constructCartoonTipLabelPaths(RootedTree tree, Node node,
			double xPosition, double[] yPosition, TreeLayoutCache cache) {

		if (!tree.isExternal(node)) {
			for (Node child : tree.getChildren(node)) {
				constructCartoonTipLabelPaths(tree, child, xPosition,
						yPosition, cache);
			}
		} else {
			...
			if (tipLabelPosition == TipLabelPosition.FLUSH) {
				...
				tipLabelPath = new Line2D.Double(transformedNodePoint,
						transform(xPosition + 1.0, yPosition[0]));

			} else if (tipLabelPosition == TipLabelPosition.RADIAL) {

				tipLabelPath = new Line2D.Double(transform(maxXPosition,
						yPosition[0]), transform(maxXPosition + 1.0,
						yPosition[0]));

				Line2D calloutPath = new Line2D.Double(transformedNodePoint,
						transform(maxXPosition, yPosition[0]));
	...
	private Point2D constructCollapsedNode(RootedTree tree, Node node,
			double xPosition, TreeLayoutCache cache) {
		...
		if (tipLabelPosition == TipLabelPosition.FLUSH) {
			...
			tipLabelPath = new Line2D.Double(transformedNodePoint, transform(
					maxXPos + 1.0, yPos));
	...
	private void constructHilight(RootedTree tree, Node node, double xParent,
			double xPosition, TreeLayoutCache cache) {
	...
	private double getAngle(double y) {
		...
		double c = 1.0 - (dist < 0 ? (dist / (scale - dist))
				: (dist / (scale + dist)));
