...
public class JavaPartyStrategy<NodeType, EdgeType> implements
	 ...
	 * 
	 * @see
	 * de.parsemis.strategy.Strategy#search(de.parsemis.miner.general.Algorithm)
	 */
	public Collection<Fragment<NodeType, EdgeType>> search(
		...
		for (int i = 1; i < answers.length; ++i) {
			...
			threads[i] = /** @at i */
			new RemoteEnvironment<NodeType, EdgeType>(
					LocalEnvironment.env(this), i).createJPThread(algo, sl,
					answers[i]);
		}
