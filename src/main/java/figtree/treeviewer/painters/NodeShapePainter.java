...
public class NodeShapePainter extends NodePainter {
	...
	public enum ShapeType {
		CIRCLE("Circle"), RECTANGLE("Rectangle"), DIAMOND("Diamond");
	 ...
	 * @param node
	 */
	public void paint(Graphics2D g2, Node node, Point2D point,
	...
	public void paint(Graphics2D g2, Node node, Justification justification,
		...
		throw new UnsupportedOperationException(
				"This version of paint is not used in NodeShapePainter");
	}
	...
	public void setSizeAttribute(String sizeAttribute) {
		...
		sizeScale = new ContinuousScale(sizeAttribute, treePane.getTree()
				.getNodes());
		firePainterChanged();
	}
	...
	private Shape createNodeShape(Node node, double x, double y) {
		...
		if (size > 0.0) {
			...
			switch (shapeType) {
			case CIRCLE:
				return new Ellipse2D.Double(x - (size * 0.5), y - (size * 0.5),
						size, size);
			case RECTANGLE:
				return new Rectangle2D.Double(x - (size * 0.5), y
						- (size * 0.5), size, size);
			case DIAMOND:
				Shape shape = new Rectangle2D.Double(x - (size * 0.5), y
						- (size * 0.5), size, size);
				return AffineTransform.getRotateInstance(Math.PI / 4.0, x, y)
