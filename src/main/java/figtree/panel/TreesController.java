...
public class TreesController extends AbstractController {
	...
	public TreesController(final TreeViewer treeViewer) {
		...
		rootingCheck.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				if (rootingCheck.isSelected()) {
					...
					treeViewer
							.setRootingType(TreePane.RootingType.USER_ROOTING);
				}
		...
		orderCombo.setSelectedItem(treeViewer.isOrderBranchesOn() ? treeViewer
				.getBranchOrdering().ordinal() + 1 : 0);
		orderCombo.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent itemEvent) {
				if (orderCombo.getSelectedIndex() == 0) {
					...
					treeViewer.setBranchOrdering(SortedRootedTree.BranchOrdering
							.values()[orderCombo.getSelectedIndex() - 1]);
				}
			}
		});
		...
		transformCombo
				.setSelectedItem(treeViewer.isOrderBranchesOn() ? treeViewer
						.getBranchTransform().ordinal() + 1 : 0);
			...
			public void itemStateChanged(ItemEvent itemEvent) {
				if (transformCombo.getSelectedIndex() == 0) {
					...
					treeViewer.setBranchTransform(TransformedRootedTree.Transform
							.values()[transformCombo.getSelectedIndex() - 1]);
				}
			}
		});
	...
	public void getSettings(Map<String, Object> settings) {
		...
		settings.put(CONTROLLER_KEY + "." + ROOTING_TYPE_KEY,
				TreePane.RootingType.MID_POINT.toString());
	}
