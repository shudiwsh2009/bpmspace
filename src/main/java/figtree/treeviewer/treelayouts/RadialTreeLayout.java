...
public class RadialTreeLayout extends AbstractTreeLayout {
	...
	public Line2D getAxisLine(double height) {
		throw new UnsupportedOperationException(
				"Method getHeightLine() is not supported in this TreeLayout");
	}

	public Shape getHeightArea(double height1, double height2) {
		throw new UnsupportedOperationException(
				"Method getHeightArea() is not supported in this TreeLayout");
	}
	...
	private Point2D constructNode(RootedTree tree, Node node,
		...
		Point2D nodePoint = new Point2D.Double(xPosition
				+ (length * directionX), yPosition + (length * directionY));

		// System.out.println("Node: " + Utils.DEBUGsubTreeRep(tree, node) +
		// " at " + nodePoint);
		...
		if (!tree.isExternal(node)) {
			...
			for (Node child : children) {
				leafCounts[i] = jebl.evolution.trees.Utils
						.getExternalNodeCount(tree, child);
			...
			for (i = 0; i < children.size(); ++i) {
				...
				Line2D branchLine = new Line2D.Double(childPoint.getX(),
						childPoint.getY(), nodePoint.getX(), nodePoint.getY());
			...
			Point2D nodeLabelPoint = new Point2D.Double(xPosition
	...
	private void constructHilight(RootedTree tree, Node node,
			double angleStart, double angleFinish, double xPosition,
			double yPosition, double length, TreeLayoutCache cache) {
