...
public class TreePaneSelector implements MouseListener, MouseMotionListener,
	...
	public enum ToolMode {
		SELECT, ROOTING, CARTOONING, COLLAPSING, ROTATING, ANNOTATING, COLOURING
	};
	...
	private void setupCursor() {
		if (toolMode != ToolMode.SELECT) {
			treePane.setCursor(java.awt.Cursor
	...
	public void mouseClicked(MouseEvent mouseEvent) {
		...
		if (toolMode == ToolMode.ROOTING) {
			Node node = treePane.getNodeAt((Graphics2D) treePane.getGraphics(),
					mouseEvent.getPoint());
			if (node != null) {
			...
			Node selectedNode = treePane.getNodeAt(
					(Graphics2D) treePane.getGraphics(), mouseEvent.getPoint());
			...
			switch (mode) {
			case NODE:
				treePane.addSelectedNode(selectedNode, invertSelection,
						extendSelection);
				break;
			case CLADE:
				treePane.addSelectedClade(selectedNode, invertSelection,
						extendSelection);
				break;
			case TAXA:
				treePane.addSelectedTip(selectedNode, invertSelection,
						extendSelection);
				break;
			default:
				throw new IllegalArgumentException("Unknown SelectionMode: "
						+ selectionMode.name());
			}
	...
	public void mousePressed(MouseEvent mouseEvent) {
		...
		dragPoint = new Point2D.Double(mouseEvent.getPoint().getX(), mouseEvent
				.getPoint().getY());
	}
	...
	public void mouseReleased(MouseEvent mouseEvent) {
		...
		if (dragMode == DragMode.SELECT) {
			if (treePane.getDragRectangle() != null) {
				Set<Node> selectedNodes = treePane.getNodesAt(
						(Graphics2D) treePane.getGraphics(), treePane
								.getDragRectangle().getBounds());
				...
				for (Node selectedNode : selectedNodes) {
					...
					case CLADE:
						treePane.addSelectedClade(selectedNode,
								invertSelection, extendSelection);
						break;
					case TAXA:
						treePane.addSelectedTip(selectedNode, invertSelection,
								extendSelection);
	...
	public void mouseDragged(MouseEvent mouseEvent) {
		...
		if (dragMode == DragMode.SCROLL) {
			// Calculate how far the mouse has been dragged from the point
			// clicked in
			// mousePressed, above.
			...
			double x1 = Math
					.min(dragPoint.getX(), mouseEvent.getPoint().getX());
			double y1 = Math
					.min(dragPoint.getY(), mouseEvent.getPoint().getY());
			double x2 = Math
					.max(dragPoint.getX(), mouseEvent.getPoint().getX());
			double y2 = Math
					.max(dragPoint.getY(), mouseEvent.getPoint().getY());
			treePane.setDragRectangle(new Rectangle2D.Double(x1, y1, x2 - x1,
					y2 - y1));
