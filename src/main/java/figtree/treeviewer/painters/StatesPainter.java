...
public class StatesPainter extends BasicLabelPainter {
	...
	public StatesPainter(String[] annotationNames,
			DiscreteColourDecorator[] decorators) {
	...
	public Rectangle2D calibrate(Graphics2D g2, Node node) {
		...
		return new Rectangle2D.Double(0.0, 0.0, getPreferredWidth(),
				getPreferredHeight());
	}
	...
	public void paint(Graphics2D g2, Node node, Justification justification,
			Rectangle2D bounds) {
		...
		case LEFT: {
			super.paint(
					...
					node,
					justification,
					new Rectangle2D.Double(bounds.getX() + width,
							bounds.getY(), bounds.getWidth() - width, bounds
									.getHeight()));
			...
			for (String name : annotationNames) {
				...
				if (item != null) {
					...
					Rectangle2D block = new Rectangle2D.Double(x, yOffset
					...
					g2.drawString(
							...
							(float) (x + 1),
							(float) (yOffset - (blockSize / 2.0)
									+ fm.getAscent() + 1));
					g2.setPaint(Color.black);
					g2.setFont(new Font("helvetica", Font.PLAIN, 9));
					g2.drawString(Integer.toString(state), (float) x,
							(float) (yOffset - (blockSize / 2.0) + fm
									.getAscent()));
				}
		...
		case RIGHT: {
			super.paint(
					...
					node,
					justification,
					new Rectangle2D.Double(bounds.getX(), bounds.getY(), bounds
							.getWidth() - width, bounds.getHeight()));
		...
		default:
			throw new IllegalArgumentException(
					"Unrecognized alignment enum option");
		}
