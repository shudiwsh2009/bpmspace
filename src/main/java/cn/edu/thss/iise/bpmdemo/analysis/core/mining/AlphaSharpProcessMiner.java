...
import org.processmining.mining.petrinetmining.TSLogAbstraction;
...
public class AlphaSharpProcessMiner extends LogRelationBasedAlgorithm implements
		ConvertingPlugin {
		...
		for (int i = 0; i < nme; i++) {
			...
			for (int j = 0; j < ialPred.size(); j++) {
				...
				for (int k = 0; k < ialPred.size() && !isDone; k++) {
					...
					// if tj --> t, tk \> t, i\|| t, there is an inv task from
					// tj to i
