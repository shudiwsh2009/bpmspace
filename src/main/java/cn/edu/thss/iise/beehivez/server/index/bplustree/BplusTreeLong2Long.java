...
/// <summary>
/// Bplustree mapping longs to longs (seek positions in file indexed).
...
/// </summary>
public class BplusTreeLong2Long {
	...
	// header consists of
	// prefix | version | node size | culture id | buffer number of
	// root | buffer number of free list head
	int headersize = HEADERPREFIX.length + 1 + BufferFile.INTSTORAGE * 2
			+ BufferFile.LONGSTORAGE * 2;
	...
	private BplusTreeLong2Long(java.io.RandomAccessFile fromfile, int NodeSize,
			int fifoLimit, long StartSeek, int CultureId) throws Exception {
	...
	void SanityCheck() throws Exception {
		...
		int keystorage = BufferFile.LONGSTORAGE;
	...
	private BplusTreeLong2Long(java.io.RandomAccessFile fromfile, int NodeSize,
		...
		this(fromfile, NodeSize, 100, (long) 0, CultureId);
	}

	public static BplusTreeLong2Long SetupFromExistingStream(
			java.io.RandomAccessFile fromfile, int fifoLimit) throws Exception {
		return SetupFromExistingStream(fromfile, fifoLimit, (long) 0);
	}

	public static BplusTreeLong2Long SetupFromExistingStream(
			java.io.RandomAccessFile fromfile, int fifoLimit, long StartSeek)
			throws Exception {
		// int dummyId = System.Globalization.CultureInfo.InvariantCulture.LCID;
		BplusTreeLong2Long result = new BplusTreeLong2Long(fromfile, 100,
				fifoLimit, StartSeek, INVARIANTCULTUREID); // dummy values for
		// nodesize, keysize
		result.readHeader();
	...
	public static BplusTreeLong2Long InitializeInStream(
			java.io.RandomAccessFile fromfile, int NodeSize, int fifoLimit)
			throws Exception {
		// int dummyId = System.Globalization.CultureInfo.InvariantCulture.LCID;
		return InitializeInStream(fromfile, NodeSize, fifoLimit,
				INVARIANTCULTUREID);
	}
	...
	public static BplusTreeLong2Long InitializeInStream(
		...
		return InitializeInStream(fromfile, NodeSize, fifoLimit, CultureId,
				(long) 0);
	}
	...
	public static BplusTreeLong2Long InitializeInStream(
		...
		BplusTreeLong2Long result = new BplusTreeLong2Long(fromfile, NodeSize,
				fifoLimit, StartSeek, CultureId);
		result.setHeader();
	...
	public void RemoveKey(long key) throws Exception {
		if (this.root == null) {
	...
	public long get(long key) throws Exception {
		// long valueFound;
	...
	public void set(long key, long value) throws Exception {
		if (!BplusNode.KeyOK(key, this)) {

			throw new BplusTreeBadKeyValue(
					"null key cannot be inserted into tree: ");
		}
		...
		// insert into root...
		// root.Insert(key, value, out splitLong, out splitNode);
		root.Insert(key, value);
		long splitLong = root.splitLong;
		...
		if (splitNode != null) {
			...
			this.root = BplusNode.BinaryRoot(oldRoot, splitLong, splitNode,
					this);
		}
		if (rootinit) {
			this.rootSeek = root.DumpToFreshBuffer();
	...
	public long FirstKey() throws Exception {
		long result = -1;
		if (this.root != null) {
			...
			if (this.ContainsKey(0)) {
				...
				return this.root.FindNextKey(0);
			}
			this.ShrinkFootprint();
		}
		return result;
	...
	public long NextKey(long AfterThisKey) throws Exception {
		if (AfterThisKey == -1) {
			throw new BplusTreeBadKeyValue("cannot search for null String");
		}
		long result = this.root.FindNextKey(AfterThisKey);
		this.ShrinkFootprint();
		return result;
	}
	...
	// return result;
	// }
	public void Set(long key, Object map) throws Exception {
		// if (!(map is long))
		if (map.getClass() != Long.class) {
			throw new BplusTreeBadKeyValue(
					"only longs may be used as values in a BplusTreeLong2Long: "
							+ map);
		}
	...
	public Object Get(long key, Object defaultValue) throws Exception {
		long valueFound;
		if (this.ContainsKey(key)) {
	...
	public void deallocateBuffer(long buffernumber) throws Exception {
		...
		buffer[0] = FREE;
		BufferFile.StoreLong(this.freeHeadSeek, buffer, 1);
	...
	void readHeader() throws Exception {
		...
		int CultureId = BufferFile.RetrieveInt(header, index);
		// this.cultureContext = new
		// System.Globalization.CultureInfo(CultureId);
		if (CultureId != INVARIANTCULTUREID) {
			throw new BplusTreeException(
	...
	public byte[] makeHeader() throws Exception {
		...
		BufferFile.StoreInt(this.NodeSize, result, index);
		index += BufferFile.INTSTORAGE;
		BufferFile.StoreInt(INVARIANTCULTUREID, result, index);
		index += BufferFile.INTSTORAGE;
		BufferFile.StoreLong(this.rootSeek, result, index);
		index += BufferFile.LONGSTORAGE;
		BufferFile.StoreLong(this.freeHeadSeek, result, index);
		return result;
	}
	...
	public static class BplusNode {
		...
		BplusTreeLong2Long owner = null;
		// buffer number of this node
		public long myBufferNumber = BplusTreeLong2Long.NULLBUFFERNUMBER;
		...
		public BplusNode(BplusTreeLong2Long owner, BplusNode parent,
		...
		public long makeRoot() throws Exception {
			...
			if (this.myBufferNumber == BplusTreeLong2Long.NULLBUFFERNUMBER) {
				throw new BplusTreeException(
		...
		public void Free() throws Exception {
			...
			this.myBufferNumber = BplusTreeLong2Long.NULLBUFFERNUMBER; // don't
			// do it
			// twice...
		}
		...
		public long SanityCheck(Hashtable visited) throws Exception {
			long result = -1;
			if (visited == null) {
			...
			if (!this.isLeaf) {
				...
				for (int i = 0; i < this.Size; i++) {
					...
					long least = this.MaterializedChildNodes[i + 1]
							.SanityCheck(visited);
					if (least == -1) {
						throw new BplusTreeException(
			...
			long lastkey = this.ChildKeys[0];
			for (int i = 1; i < this.Size; i++) {
				if (this.ChildKeys[i] == -1) {
					break;
				}
				if (lastkey == this.ChildKeys[i]) {
		...
		void Destroy() {
			...
			this.myBufferNumber = BplusTreeLong2Long.NULLBUFFERNUMBER;
		...
		public static BplusNode BinaryRoot(BplusNode LeftNode, long key,
				BplusNode RightNode, BplusTreeLong2Long owner) throws Exception {
		...
		void Clear() throws Exception {
			this.ChildBufferNumbers = new long[this.Size + 1];
			this.ChildKeys = new long[this.Size];
			this.MaterializedChildNodes = new BplusNode[this.Size + 1];
			for (int i = 0; i < this.Size; i++) {
				this.ChildBufferNumbers[i] = BplusTreeLong2Long.NULLBUFFERNUMBER;
				this.MaterializedChildNodes[i] = null;
				this.ChildKeys[i] = -1;
			}
			this.ChildBufferNumbers[this.Size] = BplusTreeLong2Long.NULLBUFFERNUMBER;
		...
		// / <param name="LookPastOnly">if true and this is a leaf then look for
		// a greater value</param>
		// / <returns>lowest index of same or greater key or this.Size if no
		// greater key.</returns>
		int FindAtOrNextPosition(long CompareKey, boolean LookPastOnly)
				throws Exception {
			int insertposition = 0;
			// System.Globalization.CultureInfo culture =
			// this.owner.cultureContext;
			...
			if (this.isLeaf && !LookPastOnly) {
				...
				while (insertposition < this.Size
						&& this.ChildKeys[insertposition] != -1 &&
						// cmp.Compare(this.ChildKeys[insertposition],
						// CompareKey)<0)
						this.ChildKeys[insertposition] < CompareKey) {
					insertposition++;
				}
			} else {
		...
		// / <summary>
		// / Find the first key below atIndex, or if no such node traverse to
		// the next key to the right.
			...
			public TraverseToFollowingKey(int atIndex) throws Exception {
				...
				if (BplusNode.this.isLeaf) {
					LookInParent = (atIndex >= BplusNode.this.Size)
							|| (BplusNode.this.ChildKeys[atIndex] == -1);
				} else {
					LookInParent = (atIndex > BplusNode.this.Size)
							|| (atIndex > 0 && BplusNode.this.ChildKeys[atIndex - 1] == -1);
				}
				if (LookInParent) {
				...
				if (BplusNode.this.isLeaf) {
					...
					if (atIndex == 0
							|| BplusNode.this.ChildKeys[atIndex - 1] != -1) {
		...
		public boolean FindMatch(long CompareKey) // , out long ValueFound)
				throws Exception {
			this.LastValueFound = 0; // dummy value on failure
			BplusNode leaf;
			// int position = this.FindAtOrNextPositionInLeaf(CompareKey, out
			// leaf, false);
			...
			if (position < leaf.Size) {
				long key = leaf.ChildKeys[position];
				if ((key != -1) && key == CompareKey) // (key.equals(CompareKey)
				{
		...
		public long FindNextKey(long CompareKey) throws Exception {
			long result = -1;
			BplusNode leaf;
			// int position = this.FindAtOrNextPositionInLeaf(CompareKey, out
			// leaf, true);
			...
			leaf = f.inLeaf;
			int position = f.atPosition;
			if (position >= leaf.Size || leaf.ChildKeys[position] == -1) {
		...
		// / <param name="LookPastOnly">If true then only look for a greater
		// value, not an exact match.</param>
		// / <returns>index of match in leaf</returns>
		FindAtOrNextPositionInLeaf findAtOrNextPositionInLeaf(long CompareKey,
		...
		class FindAtOrNextPositionInLeaf {
			...
			public FindAtOrNextPositionInLeaf(long CompareKey,
					boolean LookPastOnly) throws Exception {
				...
				if (childBufferNumber == BplusTreeLong2Long.NULLBUFFERNUMBER) {
		...
		public long DumpToFreshBuffer() throws Exception {
			...
			if (oldbuffernumber != BplusTreeLong2Long.NULLBUFFERNUMBER) {
		...
		// / <param name="MergeMe">true if the node is less than half full after
		// deletion</param>
		// / <returns>null unless the smallest key under this node has changed
		// in which case it returns the smallest key.</returns>
		Delete delete(long Key) throws Exception {
		...
		public class Delete {
			...
			public Delete(long key) throws Exception {
				...
				if (deleteBufferNumber == BplusTreeLong2Long.NULLBUFFERNUMBER) {
					throw new BplusTreeException(
				...
				long delresult = deletion.smallestKey;
				...
				if (delresult != -1 && delresult == key) // delresult.equals(key)
				{
					...
					if (deleteposition == 0) {
						...
						BplusNode.this.ChildKeys[deleteposition - 1] = -1;
					} else {
						BplusNode.this.ChildKeys[deleteposition - 1] = BplusNode.this.ChildKeys[deleteposition];
					}
					if (this.smallestKey != -1 && this.smallestKey == key) // result.equals(key)
					{
					...
					BplusNode.this.ChildKeys[BplusNode.this.Size - 1] = -1;
					...
					BplusNode.this.ChildBufferNumbers[BplusNode.this.Size] = BplusTreeLong2Long.NULLBUFFERNUMBER;
				...
				else if (delresult != -1 && deleteposition > 0) {
					if (delresult != key) // !delresult.equals(key)
					{
			...
			public long DeleteLeaf(long key) throws Exception {
				this.smallestKey = -1;
				this.MergeMe = false;
				boolean found = false;
				...
				for (int i = 0; i < BplusNode.this.ChildKeys.length; i++) {
					long thiskey = BplusNode.this.ChildKeys[i];
					// use comparison, not equals, in case different Strings
					// sometimes compare same
					if (thiskey != -1 && thiskey == key) // thiskey.equals(key)
					{
						found = true;
						deletelocation = i;
						break;
				...
				// this.ChildBufferNumbers[endlocation+1] =
				// BplusTreeLong2Long.NULLBUFFERNUMBER;
				if (BplusNode.this.SizeInUse() < BplusNode.this.Size / 2) {
					this.MergeMe = true;
				}
				if (deletelocation == 0) {
		...
		long LeastKey() throws Exception {
			long result = -1;
			if (this.isLeaf) {
		...
		public static long Merge(BplusNode left, long KeyBetween,
			...
			if (left.isLeaf || right.isLeaf) {
				...
				long rightLeastKey = right.ChildKeys[0];
				return rightLeastKey;
			}
			// merge non-leaves
			// DeleteRight = false;
			long[] allkeys = new long[left.Size * 2 + 1];
			long[] allseeks = new long[left.Size * 2 + 2];
			BplusNode[] allMaterialized = new BplusNode[left.Size * 2 + 2];
			if (left.ChildBufferNumbers[0] == BplusTreeLong2Long.NULLBUFFERNUMBER
					|| right.ChildBufferNumbers[0] == BplusTreeLong2Long.NULLBUFFERNUMBER) {
				throw new BplusTreeException(
			...
			long rightLeastKey = allkeys[leftcontent];
			int outputindex = 0;
		...
		public static void MergeLeaves(BplusNode left, BplusNode right) // , out
				...
				throws Exception {
			// DeleteRight = false;
			long[] allkeys = new long[left.Size * 2];
		...
		// foreach (String thiskey in this.ChildKeys)
		// {
		// // use comparison, not equals, in case different Strings sometimes
		// compare same
		// if (thiskey!=null && this.owner.Compare(thiskey, key)==0) //
		// thiskey.equals(key)
		// {
		// found = true;
		// break;
		...
		// {
		// result = this.ChildKeys[0];
		// // this is only relevant for the case of 2-3 trees (empty leaf after
		// deletion)
		// if (result==null)
		// {
		...
		// / <param name="position">position associated with key in external
		// structur</param>
		// / <param name="splitLong">if not null then the smallest key in the
		// new split leaf</param>
		// / <param name="splitNode">if not null then the node was split and
		// this is the leaf to the right.</param>
		// / <returns>null unless the smallest key under this node has changed,
		// in which case it returns the smallest key.</returns>
		public long Insert(long key, long position) // , out String
				// splitLong, out
				// BplusNode splitNode)
				throws Exception {
			this.splitLong = -1;
			...
			long childSplitLong;
			long childInsert = InsertChild.Insert(key, position); // , out
			// childSplitString,
			// out
			// childSplit);
			childSplit = InsertChild.splitNode;
			childSplitLong = InsertChild.splitLong;
			InsertChild.splitNode = null;
			InsertChild.splitLong = -1;
			...
			if (childSplit != null) {
				...
				if (this.ChildBufferNumbers[this.Size] != BplusTreeLong2Long.NULLBUFFERNUMBER) {
					dosplit = true;
				...
				this.ChildKeys[newChildPosition - 1] = childSplitLong;
				// this.MaterializedChildNodes[newChildPosition] = childSplit;
				// this.ChildBufferNumbers[newChildPosition] =
				// childSplit.myBufferNumber;
				childSplit.Reparent(this, newChildPosition);
				// split, if needed
				if (dosplit) {
					int splitpoint = this.MaterializedChildNodes.length / 2 - 1;
					this.splitLong = this.ChildKeys[splitpoint];
					...
					long[] keys = this.ChildKeys;
					// repair the expanded node
					this.ChildKeys = new long[this.Size];
					this.MaterializedChildNodes = new BplusNode[this.Size + 1];
					this.ChildBufferNumbers = new long[this.Size + 1];
					this.Clear();
					// Array.Copy(materialized, 0, this.MaterializedChildNodes,
					// 0, splitpoint+1);
					// Array.Copy(buffernumbers, 0, this.ChildBufferNumbers, 0,
					// splitpoint+1);
					...
					// Array.Copy(materialized, splitpoint+1,
					// splitNode.MaterializedChildNodes, 0, remainingKeys+1);
					// Array.Copy(buffernumbers, splitpoint+1,
					// splitNode.ChildBufferNumbers, 0, remainingKeys+1);
					...
					// Array.Copy(keys, splitpoint+1, splitNode.ChildKeys, 0,
					// remainingKeys);
			...
			return -1; // no change in smallest key
		}
		...
		public long InsertLeaf(long key, long position) // , out String
				// splitLong, out
				// BplusNode
				// splitNode)
				throws Exception {
			...
			if (insertposition >= this.Size) {
				...
				if (this.ChildKeys[insertposition] == -1
						|| this.ChildKeys[insertposition] == key) // this.ChildKeys[insertposition].equals(key)
				{
			...
			long nextkey = this.ChildKeys[insertposition];
			...
			if (dosplit) {
				...
				// Array.Copy(this.ChildBufferNumbers, splitpoint,
				// splitNode.ChildBufferNumbers, 0, splitlength);
				// Array.Copy(this.ChildKeys, splitpoint, splitNode.ChildKeys,
				// 0, splitlength);
				// Array.Copy(this.MaterializedChildNodes, splitpoint,
				// splitNode.MaterializedChildNodes, 0, splitlength);
				...
				splitLong = splitNode.ChildKeys[0];
				...
				// splitpoint);
				// Array.Copy(keys, 0, this.ChildKeys, 0, splitpoint);
				// Array.Copy(nodes, 0, this.MaterializedChildNodes, 0,
				// splitpoint);
				for (int i = 0; i < splitpoint; i++) {
					this.ChildBufferNumbers[i] = buffernumbers[i];
					this.ChildKeys[i] = keys[i];
		...
		void PrepareForSplit() throws Exception {
			...
			// Array.Copy(this.ChildBufferNumbers, 0, positions, 0,
			// this.Size+1);
			positions[this.Size] = this.ChildBufferNumbers[this.Size];
			positions[this.Size + 1] = BplusTreeLong2Long.NULLBUFFERNUMBER;
			// Array.Copy(this.ChildKeys, 0, keys, 0, this.Size);
			keys[this.Size] = -1;
			// Array.Copy(this.MaterializedChildNodes, 0, materialized, 0,
			// this.Size+1);
		...
		public void Load(byte[] buffer) throws Exception {
			...
			this.isLeaf = false;
			if (indicator == BplusTreeLong2Long.LEAF) {
				this.isLeaf = true;
			} else if (indicator != BplusTreeLong2Long.NONLEAF) {
				throw new BplusTreeException(
			...
			for (int KeyIndex = 0; KeyIndex < this.Size; KeyIndex++) {
				...
				long key = BufferFile.RetrieveLong(buffer, index);
				...
				if (!this.isLeaf) {
					if (key == -1
							& seekPosition != BplusTreeLong2Long.NULLBUFFERNUMBER) {
						throw new BplusTreeException(
								"key is null but position is not " + KeyIndex);
					} else if (lastkey == -1 && key != -1) {
						throw new BplusTreeException(
		...
		// / <summary>
		// / check that key is ok for node of this size (put here for locality
		// of relevant code).
		// / </summary>
		// / <param name="key">key to check</param>
		// / <param name="owner">tree to contain node containing the key</param>
		// / <returns>true if key is ok</returns>
		public static boolean KeyOK(long key, BplusTreeLong2Long owner)
				throws Exception {
			if (key == -1) {
				return false;
			}
		...
		public void Dump(byte[] buffer) throws Exception {
			...
			buffer[0] = BplusTreeLong2Long.NONLEAF;
			if (this.isLeaf) {
				buffer[0] = BplusTreeLong2Long.LEAF;
			}
			int index = 1;
			// store first seek position
			BufferFile.StoreLong(this.ChildBufferNumbers[0], buffer, index);
			index += BufferFile.LONGSTORAGE;
			// System.Text.Encoder encode =
			// System.Text.Encoding.UTF8.GetEncoder();
			...
			for (int KeyIndex = 0; KeyIndex < this.Size; KeyIndex++) {
				// store a key
				long theKey = this.ChildKeys[KeyIndex];
				BufferFile.StoreLong(theKey, buffer, index);
				index += BufferFile.LONGSTORAGE;
				// store a seek
				...
				if (lastkey == -1 && theKey != -1) {
					throw new BplusTreeException(
							"null key followed by non-null key " + KeyIndex);
				}
				lastkey = theKey;
				BufferFile.StoreLong(seekPosition, buffer, index);
				index += BufferFile.LONGSTORAGE;
			}
		}
		...
		// / <summary>
		// / Close the node:
		// / invalidate all children, store state if needed, remove materialized
		// self from parent.
		// / </summary>
		public long Invalidate(boolean destroyRoot) throws Exception {
		...
		public void AsHtml(java.io.CharArrayWriter sb) throws Exception {
			...
			if (this.isLeaf) {
				...
				sb.write("<tr><td colspan=2>nonleaf " + this.indexInParent
						...
						+ "</td></tr>\r\n");
				if (this.ChildBufferNumbers[0] != BplusTreeLong2Long.NULLBUFFERNUMBER) {
					this.MaterializeNodeAtIndex(0);
