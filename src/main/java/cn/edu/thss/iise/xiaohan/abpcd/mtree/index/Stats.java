...
public class Stats {
	...
	public List<Fragment> getNearestByRange(Fragment querygraph, double d) {
		GraphMTree.Query query = mtree.getNearestByRange(querygraph, d);
		List<Fragment> fragments = new ArrayList<Fragment>();

		// System.err.println("Query nearest fragments by range...");
		for (GraphMTree.ResultItem ri : query) {
			fragments.add(ri.data);
		}
		return fragments;
