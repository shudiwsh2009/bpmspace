...
public class LegendPainterController extends AbstractController {
	...
	public LegendPainterController(final LegendPainter legendPainter,
		...
		final String defaultFontName = PREFS.get(CONTROLLER_KEY + "."
				+ FONT_NAME_KEY, DEFAULT_FONT_NAME);
		final int defaultFontStyle = PREFS.getInt(CONTROLLER_KEY + "."
				+ FONT_STYLE_KEY, DEFAULT_FONT_STYLE);
		final int defaultFontSize = PREFS.getInt(CONTROLLER_KEY + "."
				+ FONT_SIZE_KEY, DEFAULT_FONT_SIZE);
		final int defaultSignificantDigits = PREFS.getInt(CONTROLLER_KEY + "."
				+ SIGNIFICANT_DIGITS_KEY, DEFAULT_SIGNIFICANT_DIGITS);
		final String defaultNumberFormatting = PREFS.get(CONTROLLER_KEY + "."
				+ NUMBER_FORMATTING_KEY, DEFAULT_NUMBER_FORMATTING);

		final String attribute = PREFS.get(
				CONTROLLER_KEY + "." + ATTRIBUTE_KEY, DEFAULT_ATTRIBUTE_KEY);

		legendPainter.setFont(new Font(defaultFontName, defaultFontStyle,
				defaultFontSize));
		...
		new AttributeComboHelper(attributeCombo, treeViewer)
				.addListener(new AttributeComboHelperListener() {
					...
					public void attributeComboChanged() {
						ColourDecorator colourDecorator = (ColourDecorator) colourController
								.getColourDecorator(attributeCombo, null);
		...
		// legendPainter.setDisplayAttribute((String)
		// attributeCombo.getSelectedItem());
		// }
		// });
		...
		final JLabel label1 = optionsPanel.addComponentWithLabel("Attribute:",
				attributeCombo);
		final JLabel label2 = optionsPanel.addComponentWithLabel("Font Size:",
				fontSizeSpinner);
		final JLabel label3 = optionsPanel.addComponentWithLabel("Format:",
				numericalFormatCombo);
		final JLabel label4 = optionsPanel.addComponentWithLabel(
	...
	public void setSettings(Map<String, Object> settings) {
		titleCheckBox.setSelected((Boolean) settings.get(CONTROLLER_KEY + "."
				+ IS_SHOWN));
		attributeCombo.setSelectedItem((String) settings.get(CONTROLLER_KEY
				+ "." + ATTRIBUTE_KEY));
		fontSizeSpinner.setValue((Double) settings.get(CONTROLLER_KEY + "."
				+ FONT_SIZE_KEY));
		digitsSpinner.setValue((Integer) settings.get(CONTROLLER_KEY + "."
				+ SIGNIFICANT_DIGITS_KEY));
	}

	public void getSettings(Map<String, Object> settings) {
		settings.put(CONTROLLER_KEY + "." + IS_SHOWN,
				titleCheckBox.isSelected());
		settings.put(CONTROLLER_KEY + "." + ATTRIBUTE_KEY,
				attributeCombo.getSelectedItem());
		settings.put(CONTROLLER_KEY + "." + FONT_SIZE_KEY,
				fontSizeSpinner.getValue());
		settings.put(CONTROLLER_KEY + "." + SIGNIFICANT_DIGITS_KEY,
