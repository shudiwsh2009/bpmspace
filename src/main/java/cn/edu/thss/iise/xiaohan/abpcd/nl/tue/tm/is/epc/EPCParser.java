...
public class EPCParser extends DefaultHandler {
	...
	private Arc currArc;
	private Map<Arc, String> arcToSource; // Store the nodes that arcs connect.
											// Add those last.
	private Map<Arc, String> arcToTarget; // Store the nodes that arcs connect.
											// Add those last.
	...
	public void characters(char[] arr, int start, int len) throws SAXException {
		...
		case STATE_EVENT_NAME:
			currNode.setName(currNode.getName().concat(
					new String(arr, start, len)));
		default:
			break;
		}
	}
	...
	public void startElement(String namespace, String lname, String qname,
		...
		if (qname.toLowerCase().equals("function")) {
			...
			currState = STATE_EVENT;
		} else if (qname.toLowerCase().equals("or")
				|| qname.toLowerCase().equals("xor")
