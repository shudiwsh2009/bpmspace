...
public class AnnotationDialog {
	...
	public int showDialog(List<AnnotationDefinition> definitions,
			Attributable item) {
	...
	private void setupOptionsPanel() {
		...
		if (definition == null) {
			...
			typeCombo.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent event) {
					definition = new AnnotationDefinition(nameField.getText(),
							(AnnotationDefinition.Type) typeCombo
									.getSelectedItem());
					setupValues();
				}
			});
	...
	private void setupValues() {
		...
		case BOOLEAN:
			...
			if (value != null) {
				((JComboBox) component).setSelectedIndex(value
						.equals(Boolean.TRUE) ? 0 : 1);
			}
		...
		case RANGE:
			...
			if (value != null) {
				((RealNumberField) component)
						.setValue((Double) ((Object[]) value)[0]);
				((RealNumberField) component2)
						.setValue((Double) ((Object[]) value)[1]);
			}
			break;
		default:
	...
	public Object getValue() {
		...
		switch (definition.getType()) {
		case BOOLEAN:
			return new Boolean(((JComboBox) component).getSelectedItem()
					.toString());
		case INTEGER:
