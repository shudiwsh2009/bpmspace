 ...
 * autodetects the type of attributes and then provides a colouring scheme for
 * them based on a gradient between color1 & color2.
 *
...
public class InterpolatingColourDecorator extends ContinuousColourDecorator {
	...
	public static final Color DEFAULT_COLOR_1 = Color.getHSBColor(0.01F, 0.7F,
			0.85F);
	public static final Color DEFAULT_COLOR_2 = Color.getHSBColor(0.63F, 0.7F,
			0.85F);
		...
		if (!Double.isNaN(value)) {
			if (color3 != null) {
				if (value < 0.5) {
					...
					return new Color(color2[0] * p + color1[0] * q, color2[1]
							* p + color1[1] * q, color2[2] * p + color1[2] * q,
							color2[3] * p + color1[3] * q);
				} else {
					float p = (float) (value - 0.5) * 2;
					float q = 1.0F - p;

					return new Color(color3[0] * p + color2[0] * q, color3[1]
							* p + color2[1] * q, color3[2] * p + color2[2] * q,
				...
				return new Color(color2[0] * p + color1[0] * q, color2[1] * p
						+ color1[1] * q, color2[2] * p + color1[2] * q,
						color2[3] * p + color1[3] * q);
			}
		} else {
			return null;
	...
	public void setup(String settings) {
		if (!settings.startsWith("{") || !settings.endsWith("}")) {
			throw new IllegalArgumentException(
					"InterpolatingColourDecorator settings string not in correct format");
		}
		...
		try {
			...
			if (parts2.length == 3) {
				setColours(parseColor(parts2[0]), parseColor(parts2[1]),
						parseColor(parts2[2]));
			} else {
				setColours(parseColor(parts2[0]), parseColor(parts2[1]));
			}
	...
	public String toString() {
		...
		sb.append(",#");
		sb.append(new Color(color1[0], color1[1], color1[2], color1[3])
				.getRGB());
		sb.append(",#");
		sb.append(new Color(color2[0], color2[1], color2[2], color2[3])
				.getRGB());
		if (color3 != null) {
			sb.append(",#");
			sb.append(new Color(color3[0], color3[1], color3[2], color3[3])
					.getRGB());
		}
		sb.append("}");
		return sb.toString();
