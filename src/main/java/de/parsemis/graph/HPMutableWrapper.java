...
class HPMutableWrapper<NodeType, EdgeType> extends
	...
	public Edge<NodeType, EdgeType> addEdge(
		...
		return new MyEdge<NodeType, EdgeType>(this, master.addEdgeIndex(
				nodeA.getIndex(), nodeB.getIndex(), label, direction));
	}
	...
	public Node<NodeType, EdgeType> addNode(final NodeType label) {
		final HPMutableGraph<NodeType, EdgeType> master = (HPMutableGraph<NodeType, EdgeType>) super.master;
		return new HPWrapper.MyNode<NodeType, EdgeType>(this,
				master.addNodeIndex(label));
	}
	 ...
	 * 
	 * @see
	 * de.parsemis.graph.MutableGraph#addNodeAndEdge(de.parsemis.graph.Node,
