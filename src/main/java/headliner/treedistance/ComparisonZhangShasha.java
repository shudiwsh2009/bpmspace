...
/**
 * This is an implementation of the Zhang and Shasha algorithm as described in
 * [FIXME]
 *
...
public class ComparisonZhangShasha {
	...
	public Transformation findDistance(TreeDefinition aTree,
			TreeDefinition bTree, OpsZhangShasha ops) {
			...
			for (Integer bKeyroot : bTreeKeyRoots) { // bKeyroot loop
				...
				for (int i = aLeftLeaf.get(aKeyroot); i <= aKeyroot; i++) {
					setFD(i,
							...
							getFD(i - 1, bLeftLeaf.get(bKeyroot) - 1, fD)
									+ ops.getOp(OpsZhangShasha.DELETE).getCost(
											i, 0, aTree, bTree), fD);
				...
				for (int j = bLeftLeaf.get(bKeyroot); j <= bKeyroot; j++) {
					...
					setFD(aLeftLeaf.get(aKeyroot) - 1,
							...
							getFD(aLeftLeaf.get(aKeyroot) - 1, j - 1, fD)
									+ ops.getOp(OpsZhangShasha.INSERT).getCost(
											0, j, aTree, bTree), fD);
				...
				for (int i = aLeftLeaf.get(aKeyroot); i <= aKeyroot; i++) {
					...
					for (int j = bLeftLeaf.get(bKeyroot); j <= bKeyroot; j++) {
						...
						java.lang.Math.min(// Option 1: Delete node from aTree
								getFD(i - 1, j, fD)
										+ ops.getOp(OpsZhangShasha.DELETE)
												.getCost(i, 0, aTree, bTree),
						...
						if ((aLeftLeaf.get(i) == aLeftLeaf.get(aKeyroot))
							...
							distance[i][j] = java.lang.Math
									.min(min,
											getFD(i - 1, j - 1, fD)
													+ ops.getOp(
															OpsZhangShasha.RENAME)
															.getCost(i, j,
																	aTree,
																	bTree));
	...
	/**
	 * The initiating call should be to the root node of the tree. It fills in
	 * an nxn (hash) table of the leftmost leaf for a given node. It also
	 * compiles an array of key roots. The integer values IDs must come from the
	 * post-ordering of the nodes in the tree.
	 */
	private void findHelperTables(TreeDefinition someTree,
	...
	private void findHelperTablesRecurse(TreeDefinition someTree,
			Hashtable<Integer, Integer> leftmostLeaves,
			ArrayList<Integer> keyroots, int aNodeID) {
		...
		if (someTree.isLeaf(aNodeID)) {
			...
			for (Integer child : someTree.getChildrenIDs(aNodeID)) {
				findHelperTablesRecurse(someTree, leftmostLeaves, keyroots,
						child);
				if (!seenLeftmost) {
	...
	private void seeFD(
		...
		for (Integer i : new TreeSet<Integer>(forestDistance.keySet())) {
			System.out.print(i + ": ");
			for (Integer j : new TreeSet<Integer>(forestDistance.get(i)
					.keySet())) {
				System.out
						.print(forestDistance.get(i).get(j) + "(" + j + ")  ");
			}
			System.out.println("");
		}
	}
	 ...
	 */
	private void setFD(int a, int b, double aValue,
			Hashtable<Integer, Hashtable<Integer, Double>> forestDistance) {
