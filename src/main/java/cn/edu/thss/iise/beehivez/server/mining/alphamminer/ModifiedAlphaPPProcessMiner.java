...
public class ModifiedAlphaPPProcessMiner implements MiningPlugin {
	ArrayList alT_log = new ArrayList();
	ArrayList alL1L = new ArrayList(); // 长度为1的循环中的任务，
	...
	private HashMap<String, ArrayList<AXYB>> invTaskWithAXYB;

	public ModifiedAlphaPPProcessMiner() {
	}
	...
	public MiningResult mine(LogReader log) {
		...
		progress.setMinMax(0, 16);
		MiningResult result = mineIt(log, progress);
		progress.close();
		return result;
	}
	...
	private MiningResult mineIt(LogReader log, Progress progress) {
		// scan the workflow log and computer T_log, L1L and T_prime (steps 1 to
		// 3)
		...
		MiningResult mr = constructWFNet(log);
	 ...
	 * constructWFNet
	 */
	private MiningResult constructWFNet(LogReader log) {
		Hashtable htTrans = new Hashtable();
		// generate the final petri net from the mined transitions, places and
		// arcs
	...
	/**
	 * t is a invisible task. the x and y is the tasks that may be skipped by
	 * the transition t,
	 * 
	 * @param t
	 * @param availableX
	 * @param availableY
	 */
	private void getAvailableXY(Transition t, HashSet<String> availableX,
			HashSet<String> availableY, HashSet<String> availableA,
			HashSet<String> availableB) {
	...
	private void getInvTaskAXYB(PetriNet petrinet) {
		invTaskWithAXYB = new HashMap<String, ArrayList<AXYB>>();
		for (Transition transition : petrinet.getTransitions()) {
			if (transition.getLogEvent() != null)
				continue;
			HashSet<String> availableX = new HashSet<String>();
			HashSet<String> availableY = new HashSet<String>();
			HashSet<String> availableA = new HashSet<String>();
			HashSet<String> availableB = new HashSet<String>();
			getAvailableXY(transition, availableX, availableY, availableA,
					availableB);
			ArrayList<AXYB> _axybs = new ArrayList<AXYB>();
			for (AXYB axyb : invTaskAXYB) {
				switch (axyb.invTasktype) {
				case AXYB.START:
					if (availableA.size() == 0) // 首先保证transition是一个start类型的
					{
	...
	/*
	 * private void checkCosexist(PetriNet petrinet, LogReader log) {
	 * ArrayList<CoExistAutomaton> automatons = new
	 * ArrayList<CoExistAutomaton>(); for (int i=0;
	 * i<petrinet.getTransitions().size(); i++) { Transition t1 =
	 * petrinet.getTransitions().get(i); if (t1.getLogEvent() !=null) continue;
	 * for (int j=i+1; j<petrinet.getTransitions().size(); j++) { Transition t2
	 * = petrinet.getTransitions().get(j); if(t2.getLogEvent() != null)
	 * continue; //判断t1和t2并行 String t1Name = t1.getIdentifier() +"\0auto";
	 * String t2Name = t2.getIdentifier() +"\0auto"; if
	 * (rmRelation.isNormPara(t1Name, t2Name)) { ArrayList<String> _ASet;
	 * ArrayList<String> _BSet; AXYB a_AXYB =
	 * invTaskAXYB.get(t1.getIdentifier()); AXYB b_AXYB =
	 * invTaskAXYB.get(t2.getIdentifier()); _ASet =
	 * AlphaMMinerDataUtil.union(a_AXYB.x,a_AXYB.y); _BSet =
	 * AlphaMMinerDataUtil.union(b_AXYB.x,b_AXYB.y); CoExistAutomaton
	 * ceAutomaton = new CoExistAutomaton(_ASet, _BSet, t1,t2);
	 * automatons.add(ceAutomaton); } } }
	 * 
	 * //跑一遍automaton for (ProcessInstance processInstance : log.getInstances())
	 * { AuditTrailEntryList ateList = processInstance.getAuditTrailEntryList();
	 * for (int i=0; i<ateList.size(); i++) { try { AuditTrailEntry ate =
	 * ateList.get(i); for (CoExistAutomaton automaton : automatons) {
	 * automaton.add(ate.getName()); } } catch (IndexOutOfBoundsException |
	 ...
	 * 
	 * //可以两两合并的automoton Iterator it = automatons.iterator();
	 * HashMap<Transition,Integer> disjointSet = new HashMap<Transition,
	 * Integer>(); int setCount = 0; while (it.hasNext()) { CoExistAutomaton
	 * automaton = (CoExistAutomaton) it.next(); if (!automaton.isCoexist)
	 * continue; Transition t1 = automaton.t1; Transition t2 = automaton.t2;
	 * 
	 * Integer i1,i2; i1 = disjointSet.get(t1); i2 = disjointSet.get(t2); if (i1
	 * == null && i2 == null) { setCount++; disjointSet.put(t1, setCount);
	 * disjointSet.put(t2, setCount); } else if (i1 != null && i2 == null) {
	 * disjointSet.put(t2,setCount); } else if (i1 == null && i2 != null) {
	 * disjointSet.put(t1,setCount); } else if (i1 != null && i2 != null) { if
	 * (i1.equals(i2)) continue; for (Transition transition :
	 * disjointSet.keySet()) { if (disjointSet.get(transition).equals(i2))
	 * disjointSet.put(transition, i1); } } } ArrayList<ArrayList<Transition>>
	 * transitionSets = new ArrayList<ArrayList<Transition>>(); HashMap<Integer,
	 * Integer> index = new HashMap<Integer,Integer>(); int count =0; for
	 * (Transition t : disjointSet.keySet()) { Integer i = disjointSet.get(t);
	 * if (index.containsKey(i)) { int o = index.get(i);
	 * transitionSets.get(o).add(t); } else { index.put(i, count);
	 * ArrayList<Transition> transitions = new ArrayList<Transition>();
	 * transitions.add(t); transitionSets.add(transitions); count++; } }
	 * 
	 * //for each set, combine it. for (int i=0; i<transitionSets.size(); i++) {
	 * HashSet<ModelGraphVertex> prePlaces = new HashSet<ModelGraphVertex>();
	 * HashSet<ModelGraphVertex> postPlaces = new HashSet<ModelGraphVertex>();
	 * ArrayList<Transition> transitionSet = transitionSets.get(i); Transition
	 * first = transitionSet.get(0); for (int j=1; j<transitionSet.size(); j++)
	 * { Transition t = transitionSet.get(j); HashSet<ModelGraphVertex> pre =
	 * t.getPredecessors(); HashSet<ModelGraphVertex> post = t.getSuccessors();
	 * prePlaces.addAll(pre); postPlaces.addAll(post);
	 * petrinet.delTransition(t); } for (ModelGraphVertex pre : prePlaces) { if
	 * (petrinet.getEdgesBetween((Place) pre, (Transition)first).size() == 0)
	 * petrinet.addEdge((Place) pre, (Transition)first); } for (ModelGraphVertex
	 * post : postPlaces) { if (petrinet.getEdgesBetween((Transition) first,
	 * (Place)post).size() == 0) petrinet.addEdge((Transition) first, (Place)
	 * post); } } }
	 */

	private boolean isCheckInterSequence(Transition t1, Transition t2) {
		HashSet<Place> preT1 = t1.getPredecessors();
		HashSet<Place> succT1 = t1.getSuccessors();
		HashSet<Place> preT2 = t2.getPredecessors();
		HashSet<Place> succT2 = t2.getSuccessors();
		HashSet<Transition> preT1ts = new HashSet<Transition>();
		HashSet<Transition> succT1ts = new HashSet<Transition>();
		HashSet<Transition> preT2ts = new HashSet<Transition>();
		HashSet<Transition> succT2ts = new HashSet<Transition>();
		...
		HashSet<Transition> joinPre = AlphaMMinerDataUtil
				.join(preT1ts, preT2ts);
		preT1ts = AlphaMMinerDataUtil.except(preT1ts, joinPre);
		preT2ts = AlphaMMinerDataUtil.except(preT2ts, joinPre);

		HashSet<Transition> joinSucc = AlphaMMinerDataUtil.join(succT1ts,
				succT2ts);
		...
		for (Transition ta : preT1ts) {
			...
			for (Transition tb : succT2ts) {
				if (tb.getLogEvent() == null)
					continue;
				String taskNameA = ta.getLogEvent().getModelElementName()
						+ "\0" + ta.getLogEvent().getEventType();
				String taskNameB = tb.getLogEvent().getModelElementName()
						+ "\0" + tb.getLogEvent().getEventType();
	...
	private void checkCoexist(PetriNet petrinet, LogReader log) {
		...
		for (int i = 0; i < petrinet.getTransitions().size(); i++) {
			...
			for (int j = i + 1; j < petrinet.getTransitions().size(); j++) {
				...
				if (rmRelation.isNormPara(t1Name, t2Name)) {
					...
					ArrayList<AXYB> t1axybs = invTaskWithAXYB.get(t1
							.getIdentifier());
					ArrayList<AXYB> t2axybs = invTaskWithAXYB.get(t2
							.getIdentifier());
					...
					/*
					 * AXYB a_AXYB = invTaskAXYB.get(t1.getIdentifier()); AXYB
					 * b_AXYB = invTaskAXYB.get(t2.getIdentifier()); _ASet =
					 * AlphaMMinerDataUtil.union(a_AXYB.x,a_AXYB.y); _BSet =
					 * AlphaMMinerDataUtil.union(b_AXYB.x,b_AXYB.y);
					 */
					if (!isCheckInterSequence(t1, t2)
							&& AlphaMMinerDataUtil.join(_ASet, _BSet).size() > 0)
						continue;
					CoExistAutomaton ceAutomaton = new CoExistAutomaton(_ASet,
							_BSet, t1, t2);
					automatons.add(ceAutomaton);
				}
			}
		...
		HashMap<Transition, Integer> disjointSet = new HashMap<Transition, Integer>();
		int setCount = 0;
		while (it.hasNext()) {
		...
		for (int i = 0; i < transitionSets.size(); i++) {
			...
			for (int j = 1; j < transitionSet.size(); j++) {
				...
				HashSet<ModelGraphVertex> pre = t.getPredecessors();
				HashSet<ModelGraphVertex> post = t.getSuccessors();
				prePlaces.addAll(pre);
				postPlaces.addAll(post);
				petrinet.delTransition(t);
			}
			for (ModelGraphVertex pre : prePlaces) {
				if (petrinet.getEdgesBetween((Place) pre, (Transition) first)
						.size() == 0)
					petrinet.addEdge((Place) pre, (Transition) first);
			}
			for (ModelGraphVertex post : postPlaces) {
				if (petrinet.getEdgesBetween((Transition) first, (Place) post)
						.size() == 0)
	...
	private void combinePlace(PetriNet petriNet) {
		HashMap<HashSet<Transition>, HashMap<HashSet<Transition>, ArrayList<Place>>> places = new HashMap<HashSet<Transition>, HashMap<HashSet<Transition>, ArrayList<Place>>>();
		for (Place place : petriNet.getPlaces()) {
			HashSet<Transition> preTransitions = place.getPredecessors();
			HashSet<Transition> postTransitions = place.getSuccessors();
			HashMap<HashSet<Transition>, ArrayList<Place>> singleLine = places
					.get(preTransitions);
			if (singleLine == null) {
				singleLine = new HashMap<HashSet<Transition>, ArrayList<Place>>();
		...
		for (HashMap<HashSet<Transition>, ArrayList<Place>> singleLine : places
				.values()) {
	...
	/*
	 * private void combineInvisibleTaskSup(PetriNet petriNet) //for the
	 * parallel hehe... { ArrayList<Transition> rawInvisibleTask = new
	 * ArrayList<Transition>(); ArrayList<Transition> combinedInvisibleTask =
	 * new ArrayList<Transition>(); for (Transition transition :
	 * petriNet.getTransitions()) { if (transition.getLogEvent() != null)
	 * continue; rawInvisibleTask.add(transition); }
	 * 
	 * for (int i=0; i<rawInvisibleTask.size(); i++) { Transition it =
	 * rawInvisibleTask.get(i); HashSet<Place> dsPred = it.getPredecessors();
	 * HashSet<Place> dsSucc = it.getSuccessors(); int nCombined = 0; for (int
	 * j=0; j<combinedInvisibleTask.size(); j++) { Transition iti =
	 * combinedInvisibleTask.get(j); boolean isInPred =
	 * iti.getPredecessors().containsAll(dsPred); boolean isInSucc =
	 * iti.getSuccessors().containsAll(dsSucc); if (isInPred && isInSucc) {
	 * nCombined ++; } else if (isInPred) { boolean isCombined =
	 * isCombinable(iti.getPredecessors(), it.getSuccessors(),
	 * iti.getSuccessors(), it.getSuccessors()); if (isCombined) { //将it的内容添加到
	 * //1.将it的前驱place添加到iti上 for (Place p :
	 * (HashSet<Place>)it.getPredecessors()) { if
	 * (petriNet.getEdgesBetween((Place) p, (Transition) iti).size() == 0) {
	 * petriNet.addEdge((Place) p, (Transition) iti); } } //2.将it的后继place添加到it上
	 * for (Place p : (HashSet<Place>) it.getSuccessors()) { if
	 * (petriNet.getEdgesBetween((Transition) iti, (Place) p).size() == 0) {
	 * petriNet.addEdge((Transition) iti, (Place) p); } } //3.axyb AXYB axybITI
	 * = invTaskAXYB.get(iti.getIdentifier()); AXYB axybIT =
	 * invTaskAXYB.get(it.getIdentifier()); axybITI.combine(axybIT); nCombined
	 * ++; } } else if (isInSucc) { boolean isCombined =
	 * isCombinable(it.getPredecessors(), iti.getSuccessors(),
	 * it.getPredecessors(), iti.getPredecessors()); if (isCombined) {
	 * //将it的内容添加到 //1.将it的前驱place添加到iti上 for (Place p :
	 * (HashSet<Place>)it.getPredecessors()) { if
	 * (petriNet.getEdgesBetween((Place) p, (Transition) iti).size() == 0) {
	 * petriNet.addEdge((Place) p, (Transition) iti); } } //2.将it的后继place添加到it上
	 * for (Place p : (HashSet<Place>) it.getSuccessors()) { if
	 ...
	 * = invTaskAXYB.get(iti.getIdentifier()); AXYB axybIT =
	 * invTaskAXYB.get(it.getIdentifier()); axybITI.combine(axybIT); nCombined
	 * ++; } } else { boolean isCombined = isCombinable(iti.getPredecessors(),
	 * it.getSuccessors(), iti.getSuccessors(), it.getSuccessors()); isCombined
	 * = isCombined && isCombinable(it.getPredecessors(), iti.getSuccessors(),
	 * it.getPredecessors(), iti.getPredecessors()); if (isCombined) {
	 * //将it的内容添加到 //1.将it的前驱place添加到iti上 for (Place p :
	 * (HashSet<Place>)it.getPredecessors()) { if
	 * (petriNet.getEdgesBetween((Place) p, (Transition) iti).size() == 0) {
	 ...
	 * = invTaskAXYB.get(iti.getIdentifier()); AXYB axybIT =
	 * invTaskAXYB.get(it.getIdentifier()); axybITI.combine(axybIT); nCombined
	 * ++; } } } if (nCombined > 0) { //4.删除it petriNet.delTransition(it);
	 * continue; } for (int j=i+1; j<rawInvisibleTask.size(); j++) { Transition
	 * iti = rawInvisibleTask.get(j); boolean isInPred =
	 * iti.getPredecessors().containsAll(dsPred); boolean isInSucc =
	 * iti.getSuccessors().containsAll(dsSucc); if (isInPred && isInSucc) {
	 * nCombined ++; } else if (isInPred) { boolean isCombined =
	 * isCombinable(iti.getPredecessors(), it.getSuccessors(),
	 ...
	 * invTaskAXYB.get(it.getIdentifier()); axybITI.combine(axybIT); nCombined
	 * ++; } } } if (nCombined > 0) { //4.删除it petriNet.delTransition(it); }
	 * else if (nCombined == 0) { combinedInvisibleTask.add(it); } } }
	 */

	private boolean isCombinable(HashSet<Place> sarIPredH,
			HashSet<Place> sarSuccH, HashSet<Place> sarISuccH,
			HashSet<Place> sarPredH) {
		Place[] sarIPred = sarIPredH.toArray(new Place[0]);
		Place[] sarSucc = sarSuccH.toArray(new Place[0]);
		Place[] sarISucc = sarISuccH.toArray(new Place[0]);
		Place[] sarPred = sarPredH.toArray(new Place[0]);

		for (int j = 0; j < sarIPred.length; j++) {
			HashSet<Transition> sarIPredPredH = sarIPred[j].getPredecessors();
			Transition[] sarIPredPred = sarIPredPredH
					.toArray(new Transition[0]);
			for (int k = 0; k < sarIPredPred.length; k++) {
				if (sarIPredPred[k].getLogEvent() == null)
					continue;
				for (int n = 0; n < sarSucc.length; n++) {
					HashSet<Transition> sarSuccSuccH = sarSucc[n]
							.getSuccessors();
					Transition[] sarSuccSucc = sarSuccSuccH
							.toArray(new Transition[0]);
					for (int m = 0; m < sarSuccSucc.length; m++) {
						if (sarSuccSucc[m].getLogEvent() == null)
							continue;
						String preName = sarIPredPred[k].getIdentifier() + '\0'
								+ sarIPredPred[k].getLogEvent().getEventType();
						String succName = sarSuccSucc[m].getIdentifier() + '\0'
								+ sarSuccSucc[m].getLogEvent().getEventType();
						boolean isContained = false;
						for (InvTask invTask : invTasks) {
							if (invTask.pre.contains(preName)
									&& invTask.suc.contains(succName)) {
								isContained = true;
								break;
							}
						}
						 ...
						 * sarSuccSucc.get(m)); if (!alFalseDep.contains(ps))
						 * return false;
						 */
					}
				}
		...
		for (int j = 0; j < sarIPred.length; j++)
			for (int k = 0; k < sarSucc.length; k++)
				if (existPara(sarIPred[j].getSuccessors(),
						sarSucc[k].getPredecessors()))
					return false;
		// I's Succ vs Succ
		for (int j = 0; j < sarISucc.length; j++)
			for (int k = 0; k < sarPred.length; k++)
				if (!(existPara(sarISucc[j].getPredecessors(),
						sarPred[k].getPredecessors()) || existPara(
						sarISucc[j].getSuccessors(), sarPred[k].getSuccessors())))
					return false;
		return true;

	}

	private boolean existPara(HashSet<Transition> sarI, HashSet<Transition> sarJ) {
		for (Transition ti : sarI) {
			if (ti.getLogEvent() == null)
				continue;
			for (Transition tj : sarJ) {
				if (tj.getLogEvent() == null)
					continue;
				String tName1 = ti.getIdentifier() + '\0'
						+ ti.getLogEvent().getEventType();
				String tName2 = tj.getIdentifier() + '\0'
						+ tj.getLogEvent().getEventType();
				if (rmRelation.isNormPara(tName1, tName2))
					return true;
			}
		}
		return false;
	...
	/*
	 * private void combineInvisibleTask(PetriNet petriNet) {
	 * HashMap<InvisibleAdjacent,ArrayList<Transition>> prePostInv = new
	 * HashMap<InvisibleAdjacent,ArrayList<Transition>>(); for (Transition
	 * transition : petriNet.getTransitions()) { if (transition.getLogEvent() !=
	 * null) continue; HashSet<ModelGraphVertex> prePlaces =
	 * transition.getPredecessors(); HashSet<ModelGraphVertex> postPlaces =
	 * transition.getSuccessors(); InvisibleAdjacent ia =
	 * InvisibleAdjacent.getInvisibleTask(petriNet,prePlaces,postPlaces);
	 * ArrayList<Transition> trans = prePostInv.get(ia); if (trans == null) {
	 * trans = new ArrayList<Transition>(); prePostInv.put(ia, trans); }
	 * trans.add(transition); }
	 * 
	 * 
	 * for (ArrayList<Transition> trans : prePostInv.values()) { if
	 * (trans.size()>1) { AXYB head =
	 * invTaskAXYB.get(trans.get(0).getIdentifier());
	 * 
	 * HashSet<Place> prePlaceStd = trans.get(0).getPredecessors();
	 * //只需要这些place就够了，剩下的place都是多余的！ HashSet<Place> succPlaceStd =
	 * trans.get(0).getSuccessors();
	 * 
	 * for (int i=1; i<trans.size(); i++) { Transition t = trans.get(i);
	 * 
	 * HashSet<Place> prePlace = t.getPredecessors();
	 * //只需要这些place就够了，剩下的place都是多余的！ HashSet<Place> succPlace =
	 * t.getSuccessors(); for (Place p : prePlace) if (!prePlaceStd.contains(p))
	 * petriNet.delPlace(p); for (Place p : succPlace) if
	 * (!succPlaceStd.contains(p)) petriNet.delPlace(p); AXYB axyb =
	 * invTaskAXYB.get(t.getIdentifier()); head.combine(axyb);
	 * petriNet.delTransition(t); } } }
	 * 
	 * }
	 */
	 ...
	 * 
	 * //TODO
	 * HashMap<HashSet<ModelGraphVertex>,HashMap<HashSet<ModelGraphVertex>
	 * ,ArrayList<Transition>>> prePostInv = new
	 * HashMap<HashSet<ModelGraphVertex
	 * >,HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>>>(); for
	 * (Transition transition : petrinet.getTransitions()) { if
	 * (transition.getLogEvent() != null) continue; HashSet<ModelGraphVertex>
	 * prePlaces = transition.getPredecessors(); HashSet<ModelGraphVertex>
	 * postPlaces = transition.getSuccessors();
	 * HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>> postInv =
	 * prePostInv.get(prePlaces); if (postInv == null) { postInv = new
	 * HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>>();
	 * prePostInv.put(prePlaces,postInv); }
	 * 
	 * ArrayList<Transition> trans = postInv.get(postPlaces); if (trans == null)
	 * { trans = new ArrayList<Transition>(); postInv.put(postPlaces, trans); }
	 * trans.add(transition); }
	 * 
	 * for (HashMap<HashSet<ModelGraphVertex>,ArrayList<Transition>> postInv :
	 * prePostInv.values()) { for (ArrayList<Transition> trans :
	 * postInv.values()) { if (trans.size()>1) { AXYB head =
	 * invTaskAXYB.get(trans.get(0).getIdentifier()); for (int i=1;
	 * i<trans.size(); i++) { Transition t = trans.get(i); AXYB axyb =
	 * invTaskAXYB.get(t.getIdentifier()); head.combine(axyb);
	 * petrinet.delTransition(t); } } } }
	 * 
	 * }
	 ...
	 * TODO Auto-generated method stub for (Transition transition :
	 * petrinet.getTransitions()) { boolean isInv = false; for (InvTask invTask
	 * : invTasks) if (invTask.taskName.equals(transition.getIdentifier())) {
	 * isInv = true; break; } if (!isInv) continue; //1.去除 logevent
	 * transition.setLogEvent(null); AXYB axyb =
	 * invTaskAXYB.get(transition.getIdentifier()); if (axyb == null) continue;
	 * 
	 * //a->x HashSet<ModelGraphVertex> prePlaces =
	 * transition.getPredecessors(); for (ModelGraphVertex prePlace : prePlaces)
	 * { HashSet<ModelGraphVertex> prePlacePostTasks = prePlace.getSuccessors();
	 * for (ModelGraphVertex task : prePlacePostTasks) { if
	 * (!task.getIdentifier().equals(axyb.x.get(0))) continue;
	 * 
	 * if (task.getPredecessors().size() > 1) { for (ModelGraphVertex otherPlace
	 * : (HashSet<ModelGraphVertex>) task.getPredecessors()) { if
	 * (otherPlace.equals(prePlace)) continue; if
	 * (!checkPlaceInIDW3(otherPlace)) //since only the new place in IDW3 should
	 * be adjusted. continue; if (petrinet.getEdgesBetween((Place) otherPlace,
	 * (Transition) transition).size() ==0) petrinet.addEdge((Place) otherPlace,
	 * (Transition) transition); } } } }
	 * 
	 * //y->b HashSet<ModelGraphVertex> postPlaces = transition.getSuccessors();
	 * for (ModelGraphVertex postPlace : postPlaces) { HashSet<ModelGraphVertex>
	 * postPlacePreTasks = postPlace.getPredecessors(); for (ModelGraphVertex
	 * task : postPlacePreTasks) { if
	 * (!task.getIdentifier().equals(axyb.y.get(0))) continue; if
	 * (task.getSuccessors().size() > 1) { for (ModelGraphVertex otherPlace :
	 * (HashSet<ModelGraphVertex>) task.getSuccessors()) { if
	 * (otherPlace.equals(postPlace)) continue; if
	 * (!checkPlaceInIDW3(otherPlace)) //since only the new place in IDW3 should
	 * be adjusted. continue; if (petrinet.getEdgesBetween((Transition)
	 * transition,(Place) otherPlace).size() ==0) petrinet.addEdge((Transition)
	 * transition,(Place) otherPlace); } } } }
	 * 
	 * } }
	 */
	/*
	 * 重构 adjustNonfreeChoice (对应的是split) 对于一个不可见任务A,如何重构:
	 * 1.判断A中有多少组axyb需要添加新的place,(无论是x或者y都算数)，记录个数为num 1.1 如果num=0,那么不许要split
	 * 1.2 如果num=1,那么直接添加需要的place即可, 1.3如果num>1,split (num-1)次，每一次分别添加所需要的place
	 */
	...
	private void adjustNonfreeChoiceInvisibleTask(PetriNet petriNet) {
		...
		for (Transition transition : invTransitions) {
			ArrayList<AXYB> tAXYBs = invTaskWithAXYB.get(transition
					.getIdentifier());
			ArrayList<AXYB> needToAdjustAXYBs = getNeedtoAjustAXYBs(tAXYBs,
					alID_W3);
			...
			if (num == 0) {
				...
				for (int i = 0; i < num; i++) {
					...
					if (axyb.invTasktype == AXYB.SKIP) {
						HashSet<Place> prePlaces = transition.getPredecessors();
						HashSet<Place> succPlaces = transition.getSuccessors();
						for (Place p : prePlaces) {
							HashSet<Transition> prePlaceSuccT = p
									.getSuccessors();
						...
						for (Place p : succPlaces) {
							HashSet<Transition> succPlacePreT = p
									.getPredecessors();
						...
						HashSet<Place> succPlaces = transition.getSuccessors();
						for (Place p : prePlaces) {
							HashSet<Transition> prePlacePreT = p
									.getPredecessors();
						...
						for (Place p : succPlaces) {
							HashSet<Transition> succPlacesuccT = p
									.getSuccessors();
						...
						prePlace = (Place) transition.getPredecessors()
								.iterator().next();
						...
						succPlace = (Place) transition.getSuccessors()
								.iterator().next();
	...
	private void adjustSingleTransition(AXYB axyb, Transition transition,
			PetriNet petriNet) {
		if (axyb.invTasktype == AXYB.SKIP) {
			...
			HashSet<Place> succPlaces = transition.getSuccessors();
			for (Place p : prePlaces) {
				HashSet<Transition> prePlaceSuccT = p.getSuccessors();
				for (Transition t : prePlaceSuccT) {
					if (t.getIdentifier().equals(axyb.x))
						xTransition = t;
				}
			...
			for (Place p : succPlaces) {
				HashSet<Transition> succPlacePreT = p.getPredecessors();
				for (Transition t : succPlacePreT) {
					if (t.getIdentifier().equals(axyb.y))
						yTransition = t;
				}
			...
			if (xTransition == null || yTransition == null)
				return;
			for (Place p : (HashSet<Place>) xTransition.getPredecessors()) {
				if (!transition.getPredecessors().contains(p))
					petriNet.addEdge((Place) p, (Transition) transition);
			}
			for (Place p : (HashSet<Place>) yTransition.getSuccessors()) {
			...
			if (aTransition == null || bTransition == null)
				return;
			for (Place p : (HashSet<Place>) aTransition.getSuccessors()) {
				if (!transition.getPredecessors().contains(p))
					petriNet.addEdge((Place) p, (Transition) transition);
			}
			for (Place p : (HashSet<Place>) bTransition.getPredecessors()) {
				if (!transition.getSuccessors().contains(p))
					petriNet.addEdge((Transition) transition, (Place) p);
			}
		} else if (axyb.invTasktype == AXYB.START) {
	...
	private ArrayList<AXYB> getNeedtoAjustAXYBs(ArrayList<AXYB> tAXYBs,
		...
		for (AXYB axyb : tAXYBs) {
			if (axyb.invTasktype == AXYB.SKIP || axyb.invTasktype == AXYB.END
					|| axyb.invTasktype == AXYB.START) {
				String xName = axyb.x;
				String yName = axyb.y;
				boolean tag1 = false;
				boolean tag2 = false;
	...
	private boolean checkPlaceInIDW3(ModelGraphVertex otherPlace) {
		Set<String> pres = alID_W3.htPred.keySet();
		for (String pre : pres) {
			ArrayList<String> succs = (ArrayList<String>) alID_W3.htPred
					.get(pre);
			String ppre = pre.substring(0, pre.indexOf("\0"));
			for (String succ : succs) {
				String ssucc = succ.substring(0, succ.indexOf("\0"));
				HashSet<Transition> preTransitions = otherPlace
						.getPredecessors();
				HashSet<Transition> succTransitions = otherPlace
						.getSuccessors();
				boolean a = false;
				boolean b = false;
				for (Transition t : preTransitions) {
					if (t.getIdentifier().equals(ppre))
						a = true;
				}
				for (Transition t : succTransitions) {
					if (t.getIdentifier().equals(ssucc))
						b = true;
				}
				if (a && b)
					return true;
	...
	private void applyingTheoremThree() {
		...
		for (int i = 0; i < alOrJoin.size(); i++) {
			...
			for (int j = 0; j < arCommSucc.length; j++) {
				...
				// enumerate all tasks ai<|bj, divide all these tasks in A and B
				// by ||
				for (int k = 0; k < alOrSplit.size(); k++) {
					String t = (String) alOrSplit.get(k);
					/*
					 * if(t.equals(a) || t.equals(b)) continue;
					 */
				...
				for (int k = 0; k < alGroupA.length; k++) {
					...
					for (int m = 0; m < alGroupB.length; m++) {
						...
						if (rmRelation.isFitableAB(alA, alB)) {
							// A' and its input tasks
							ArrayList alAp = rmRelation
									.calculatePrime(alA, alB);
							ArrayList alInputOfAp = rmRelation
									.calculateInput(alAp);
							alInputOfAp.addAll(alInputOfA);
							// B' and its input tasks
							ArrayList alBp = rmRelation
									.calculatePrime(alB, alA);
							ArrayList alInputOfBp = rmRelation
									.calculateInput(alBp);
	...
	private void eliminateRDByRule1() {
		...
		Message.add("ID_W2 after eliminated:" + alID_W2);
	}

	private void eliminateRDByRule2() {
		alID_W3.eliminateByRuleTwo(rmRelation);

		Message.add("ID_W3 after eliminated:" + alID_W3);
	}
					...
					if (ds.containedInA(t)) {
						...
						for (int k = 0; k < alOrSplit.size(); k++) {
							...
							for (int l = 0; l < arB.length; l++) {
								String b = arB[l];
								// test if all tasks in arY and a,b can satisfy
								// Theorem 2
	...
	private void miningUsingAlpha() {
		...
		for (int i = 0; i < alTSucc.size(); i++) {
			...
			for (int j = 0; j < arPlaces.length; j++) {
				// eliminate these place that are input places of any task in
				// IDW1
	...
	private DoubleSets[] genInputPlaces(String t) {
		...
		// add t to Succ
		alSucc.add(0, t);
		// construct places (A, B) such that A<=*t, t in B, a in A and b in
		// B=>a->b, a1,a2 in A=>a1#a2, b1,b2 in B=>b1#b2
		for (int j = 0; j < arPred.length; j++) {
		...
		for (int l = 0; l < arA1.length; l++) {
			...
			if (!dsA2B2.containedInA(a)) {
				...
				for (int m = 0; m < arA2.length; m++) {
					...
					// if (rmRelation.isNormPara(arA2[m], a) ||
					// rmRelation.isFollowedBy(arA2[m], a))
					{
						isExist = true;
						break;
					}
				...
				if (!isExist) {
					...
					for (int m = 0; m < alB2.size(); m++) {
						...
						if (rmRelation.isNormPara(b_prime, a))
							continue;
						ImplicitDependency ido = new ImplicitDependency(a,
								b_prime);
						// surely distinguish
	...
	private void induceReachable(LogReader log) {
		/*
		 * Which tasks need to identify reachable relations? There are five case
		 * listed below now.
		 */
		...
		for (int i = 0; i < alT_prime.size(); i++) {
			...
			if ((type & Relation.ORJOINDIFFOUT) > 0
					&& !alOrJoinDiffOut.contains(t)) {
		...
		for (int i = 0; i < alOrJoin.size(); i++) {
			...
			for (int j = 0; j < alAndSplit.size(); j++) {
				...
				for (int k = 0; k < arSucc.length; k++) {
					...
					if (/* !pred.equals(succ) && */!rmRelation.isBefore(pred,
							succ)) {
						rm.add(pred, succ);
					}
				}
		...
		HashMap<String, HashMap<String, ArrayList<InvTask>>> invisibleTaskMap = new HashMap<String, HashMap<String, ArrayList<InvTask>>>(); // The
																																			// beginInvisibleTask,
																																			// theEndInvisibleTask
																																			// ==>InvisibleTask;
		...
		for (int i = 0; i < np && !rm.isFinished(); i++) {
			...
			ArrayList<AuditTrailEntryList> atels = adjustInvisibleTask(atel,
					invisibleTaskMap);
	...
	private void beginAdjustInvisibleTask(
			HashMap<String, HashMap<String, ArrayList<InvTask>>> invisibleTaskMap) {
		...
		for (InvTask invTask : invTasks) {
			...
			for (String preTask : preTasks)
				for (String sucTask : sucTasks) {
					HashMap<String, ArrayList<InvTask>> singleLine = invisibleTaskMap
							.get(preTask);
					if (singleLine == null) {
						singleLine = new HashMap<String, ArrayList<InvTask>>();
	...
	private ArrayList<AuditTrailEntryList> adjustInvisibleTask(
			AuditTrailEntryList atel,
			HashMap<String, HashMap<String, ArrayList<InvTask>>> invisibleTaskMap) {
		...
		HashMap<Integer, ArrayList<InvTask>> posInvisibleTask = new HashMap<Integer, ArrayList<InvTask>>(); // each
																											// pos
																											// correspond
																											// with
																											// a
		...
		try {
			...
			} else // there exisits matchings..
			{
				...
				getAllTraces(-1, 0, sortedPos, result, nowList, atel,
						posInvisibleTask); // 递归来找所有的可能的trace啊
			}
		} catch (IndexOutOfBoundsException e) {
			e.printStackTrace();
		} catch (IOException e) {
	...
	private void getAllTraces(int lastStep, int step, Integer[] sortedPos,
			ArrayList<AuditTrailEntryList> result, AuditTrailEntryList nowList,
			AuditTrailEntryList basicList,
			HashMap<Integer, ArrayList<InvTask>> posInvisibleTask)
		...
		for (InvTask invTask : availableTask) {
			...
			// 递归的做
			getAllTraces(nowPos + 1, step + 1, sortedPos, result, nowList,
					basicList, posInvisibleTask);
		}
	}

	/*
	 * private AuditTrailEntryList adjustInvisibleTask(AuditTrailEntryList atel)
	 * { // TODO Auto-generated method stub AuditTrailEntryList result = null;
	 * try { result = atel.cloneInstance(); int pos = 0; while (pos
	 * <result.size()) { checkAte(result,pos); //check wether an inv-task should
	 * be insert. pos ++; }
	 * 
	 * } catch (IOException e) { e.printStackTrace(); } return result; }
	 * 
	 * private void checkAte(AuditTrailEntryList result, int pos) throws
	 * IndexOutOfBoundsException, IOException { for (InvTask invTask : invTasks)
	 * {
	 * 
	 * ArrayList<String> preTasks = invTask.pre; ArrayList<String> sucTasks =
	 * invTask.suc; String preLogName; String postLogName;
	 * 
	 * AuditTrailEntry postAte = result.get(pos); postLogName =
	 * postAte.getElement() + "\0" + postAte.getType();
	 * 
	 * if (pos == 0) //第一只 { if (invTask.type != InvTask.START) //不是start的话，不予考虑
	 * continue; if (sucTasks.contains(postLogName)) //包含了，需要添加 {
	 * AuditTrailEntryImpl ate = new AuditTrailEntryImpl(); String invTaskName =
	 * invTask.taskName; int splitPos = invTaskName.indexOf("\0"); String
	 * taskElement = invTask.taskName; String taskType = "auto";
	 * ate.setElement(taskElement); ate.setType(taskType); result.insert(ate,
	 * pos); }
	 * 
	 * } else if (pos+1 == result.size() && invTask.type == InvTask.END) { if
	 * (preTasks.contains(postLogName)) //包含了，需要添加 { AuditTrailEntryImpl ate =
	 * new AuditTrailEntryImpl(); String invTaskName = invTask.taskName; int
	 * splitPos = invTaskName.indexOf("\0"); String taskElement =
	 * invTask.taskName; String taskType = "auto"; ate.setElement(taskElement);
	 * ate.setType(taskType); result.insert(ate, pos+1); } } else { if
	 * (invTask.type != InvTask.SKIP_REDO_SWITCH) continue; AuditTrailEntry
	 * preAte = result.get(pos-1); preLogName = preAte.getElement() + "\0" +
	 * postAte.getType(); if (preTasks.contains(preLogName) &&
	 * sucTasks.contains(postLogName)) //包含了，需要添加 { AuditTrailEntryImpl ate =
	 * new AuditTrailEntryImpl(); String invTaskName = invTask.taskName; int
	 * splitPos = invTaskName.indexOf("\0"); String taskElement =
	 * invTask.taskName; String taskType = "auto"; ate.setElement(taskElement);
	 * ate.setType(taskType); result.insert(ate, pos); } }
	 * 
	 * } }
	 */
			...
			for (int j = 0; j < atel.size(); j++) {
				try {
					...
					if (!last1Task.equals("") && !last1Task.equals(task)) {
						// > relation
						rmRelation
								.addRelation(last1Task, task, Relation.BEFORE);
						// before task
	...
	public String getHtmlDescription() {
		return "<h1>"
				+ getName()
				+ "</h1><p>For more information, please refer to publication:<br>&nbsp;&nbsp;&nbsp;Lijie Wen, Jianmin Wang and Jiaguang Sun<br>&nbsp;&nbsp;&nbsp;<i>Detecting Implicit Dependencies between Tasks from Event Logs</i><br>&nbsp;&nbsp;&nbsp;In Xiaofang Zhou et al.(Eds.): APWeb 2006, LNCS 3841, pp.591-603, 2006. Springer-Verlag Berlin Heidelberg 2006</p>";
	}

	public MiningResult mineMiscInfo(LogReader log, AlphaPPData alphaPPData,
			Progress progress) {
		// 1.把alphaPPData里面的变量赋值进去
	...
	public MiningResult mineAlphPPInfo(LogReader log, AlphaPPData alphaPPData,
			Progress progress) {
		alT_log = alphaPPData.alT_log;
		alL1L = alphaPPData.alL1L; // 长度为1的循环中的任务，
		alT_prime = alphaPPData.alT_prime;
		...
		eliminateL1L();
		Date d4 = new Date();
		System.out.println("eliminateL1L" + (d4.getTime() - d3.getTime()));
		// 在L1L中，因为
		// induce >> from log
		...
		Date d6 = new Date();
		System.out
				.println("applyingTheoremOne" + (d6.getTime() - d5.getTime()));
		// mining WF-net using alpha algorithm (step 9)
		...
		Date d9 = new Date();
		System.out
				.println("applyingTheoremTwo" + (d9.getTime() - d8.getTime()));
		// eliminate redundant dependency by rule 1 (step 11)
		...
		eliminateRDByRule1();
		Date d10 = new Date();
		System.out.println("eliminateRDByRule1"
				+ (d10.getTime() - d9.getTime()));
		// derive places involving implicit dependency two (steps 12 and 13)
		...
		derivePlacesForIDTwo();
		Date d11 = new Date();
		System.out.println("derivePlacesForIDTwo"
				+ (d11.getTime() - d10.getTime()));
		// apply theorem 3 (step 14)
		...
		applyingTheoremThree();
		Date d12 = new Date();
		System.out.println("applyingTheoremThree"
				+ (d12.getTime() - d11.getTime()));
		// eliminate redundant implicit dependency (step 15)
		...
		eliminateRDByRule2();
		Date d13 = new Date();
		System.out.println("eliminateRDByRule2"
				+ (d13.getTime() - d12.getTime()));
		// derive places involving implicit dependency three (steps 16 and 17)
		...
		derivePlacesForIDThree();
		Date d14 = new Date();
		System.out.println("derivePlacesForIDThree"
				+ (d14.getTime() - d13.getTime()));
		// gather all the places of the mined WF-net (step 18)
		...
		Date d15 = new Date();
		// System.out.println("addShortLoopPlaces"+(d15.getTime() -
		// d14.getTime()));
		// return the final result (steps 19-21)
...
class TripleSets extends DoubleSets {
...
class IDWManager {
	...
	protected void eliminateByRuleTwo(RelationMatrix rmRelation) {
		...
		// remove all implicit dependencies in htComposite from htPred and
		// htSucc
	 ...
	 *            ArrayList
	 */
	private void composite(String start, String end, Hashtable htComposite,
			ArrayList<String> alVisited) {
...
class RelationMatrix {
	...
	private boolean hasDiffOut(String ti, String tj) {
		...
		if (alUnion.size() != arSuccTi.length
				|| alUnion.size() != arSuccTj.length) {
			return true;
		}
	...
	public boolean isLoopSucc(String t1, String t2) {
		return isBefore(t1, t2) && !isBefore(t2, t1) || isShortLoop(t1, t2)
				|| isShortLoop(t2, t1);
	}
	...
	public boolean isLoopPara(String t1, String t2) {
		return isBefore(t1, t2) && isBefore(t2, t1)
				&& !(isShortLoop(t1, t2) || isShortLoop(t2, t1));
	}
