...
/**
 * @author JinTao
 * every index item must be expressed by a String
 * map a String to a long
 * and then relate it with a inverted index
 */
...
public class GenericInvertedIndex {
	...
	final static String pathIndexConfigFile = "PathIndexConfig.ini";
	...
	final static String bptFileSuffix = ".bpt";
	final static String fvFileSuffix = ".ifl";

	// the directory for index storage
	...
	public boolean destroy() {
		...
		if (!r) {
			System.out.println("index: " + indexDirectory + "/" + indexName
					+ " destory failed");
		}
		return r;
	}
	 ...
	 * @throws Exception
	 */
	public void createNewIndex(int bplusTreeNodeSize, int bplusTreeCacheSize,
			int invertedListSize, int invertedFileCacheSize) throws Exception {
		...
		String bptFileName = fIndexDir.getPath() + "/" + this.indexName
				+ bptFileSuffix;
		String fvFileName = fIndexDir.getPath() + "/" + this.indexName
				+ fvFileSuffix;
		this.bptFile = new RandomAccessFile(bptFileName, "rw");
		this.fvFile = new RandomAccessFile(fvFileName, "rw");
		this.bpt = BplusTreeLong2Long.InitializeInStream(this.bptFile,
				bplusTreeNodeSize, bplusTreeCacheSize);
		this.bpt.Commit();
		this.invertedList = InvertedFile.initializeInvertedFile(fvFile,
				invertedListSize, invertedFileCacheSize);
	}
	...
	public void createNewIndex() throws Exception {
		...
		String InvertedFileCacheSize = p.getProperty("InvertedFileCacheSize",
				"1000");
		fin.close();
		...
		createNewIndex(bplusTreeNodeSize, bplusTreeCacheSize, invertedListSize,
				invertedFileCacheSize);
	}
	...
	public void setupFromExistingIndex(int bplusTreeCacheSize,
			int invertedFilecacheSize) throws Exception {
		...
		this.bptFile = new RandomAccessFile(bptFileName, "rw");
		this.fvFile = new RandomAccessFile(fvFileName, "rw");
		this.bpt = BplusTreeLong2Long.SetupFromExistingStream(this.bptFile,
				bplusTreeCacheSize);
		this.invertedList = InvertedFile.setupFromExistingInvertedFile(fvFile,
				invertedFilecacheSize);
	}
