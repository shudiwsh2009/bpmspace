...
/**
 * This is a panel that has a TreeViewer and a BasicControlPalette with the
 * default Controllers and Painters.
 *
...
public class FigTreePanel extends JPanel {
	public enum Style {
		DEFAULT, SIMPLE, MINIMAL, ICARUS, ICARUS_SMALL
	}
	...
	public FigTreePanel(final TreeViewer treeViewer,
			ControlPalette controlPalette1, ControlPalette controlPalette2,
			Style style) {

		this.treeViewer = treeViewer;

		if (controlPalette1 != null) {
			controlPalette1.getPanel().setBorder(
		...
		if (controlPalette2 != null) {
			controlPalette2.getPanel().setBorder(
		...
		final SimpleLabelPainter tipLabelPainter = new SimpleLabelPainter(
				SimpleLabelPainter.PainterIntent.TIP);
		// Create a node label painter and its controller
		final SimpleLabelPainter nodeLabelPainter = new SimpleLabelPainter(
				SimpleLabelPainter.PainterIntent.NODE);
		nodeLabelPainter.setVisible(false);
		final SimpleLabelPainter branchLabelPainter = new SimpleLabelPainter(
				SimpleLabelPainter.PainterIntent.BRANCH);
		...
		case DEFAULT:
			if (controlPalette1 == null) {
				throw new IllegalArgumentException(
						"DEFAULT style requires a controlPalette");
			}
			...
			controlPalette1.addController(new TreeAppearanceController(
					treeViewer, "tipLabels", tipLabelPainter, "nodeLabels",
					nodeLabelPainter, "branchLabels", branchLabelPainter));

			controlPalette1.addController(new TreesController(treeViewer));

			controlPalette1.addController(new LabelPainterController(
					"tipLabels", tipLabelPainter, "nodeLabels",
					nodeLabelPainter, "branchLabels", branchLabelPainter,
					treeViewer));
			break;
		case SIMPLE:
			if (controlPalette1 == null) {
				throw new IllegalArgumentException(
						"SIMPLE style requires a controlPalette");
			}
			controlPalette1.addController(new TreeViewerController(treeViewer));
		...
		case ICARUS:
			...
			controlPalette1
					.addController(new TreeAppearanceController(treeViewer,
							"tipLabels", tipLabelPainter, "nodeLabels",
							nodeLabelPainter, "branchLabels",
							branchLabelPainter, true));
			...
			controlPalette2.addController(new TreeColouringController(
					treeViewer, "Clustering:"));
		...
		// controlPalette.addController(new
		// ScaleBarPainterController(scaleBarPainter));
			...
			if (attribute != null && attribute.length() > 0) {
				if (attribute.endsWith("*")) {
					...
					Decorator decorator = new InterpolatingColourDecorator(
							new ContinuousScale(attribute, nodes), new Color(
									192, 16, 0), new Color(0, 0, 0), new Color(
									0, 16, 192));
