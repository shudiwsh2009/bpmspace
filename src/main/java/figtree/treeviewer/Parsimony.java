...
public class Parsimony {
	...
	// private boolean[][] intersection; // as nodes are not guaranteed to be
	 ...
	 * If this method is repeatedly called with the same tree and patterns then
	 * only the first call will reconstruct the states and each subsequent call
	 * will return the stored states.
	 *
	 * @param tree
	...
	public Integer getState(Tree tree, Node node) {
		...
		if (!(tree instanceof RootedTree)) {
			throw new IllegalArgumentException(
					"The tree must be an instance of rooted tree");
		}
		...
		if (this.tree == null || this.tree != tree) {
			...
			if (!Utils.isBinary(this.tree)) {
				throw new IllegalArgumentException(
						"The Fitch algorithm can only reconstruct ancestral states on binary trees");
			}
	...
	/**
	 * This is the first pass of the Fitch algorithm. This calculates the set of
	 * states at each node and counts the total number of siteScores (the
	 * score). If that is all that is required then the second pass is not
	 * necessary.
	 */
	private void calculateSteps(RootedTree tree) {
		...
		// iterate in reverse - post order. State of child is gurantted to be
		// reasy before parent
			...
			if (tree.isExternal(node)) {
				...
				for (Node child : tree.getChildren(node)) {
					...
					if (first) {
						...
						intersectionOf(intersection, childStateSet,
								intersection);
					}
				}
	...
	/**
	 * The second pass of the Fitch algorithm. This reconstructs the ancestral
	 * states at each node.
	 *
	 * @param node
	 * @param parentState
	...
	private boolean[] reconstructStates2(Node node, boolean[] parentStateSet) {
		...
		if (!tree.isExternal(node)) {
			...
			boolean first = true;
			for (Node child : tree.getChildren(node)) {
				boolean[] childStateSet = reconstructStates2(child,
						nodeStateSet);
				if (first) {
					copyOf(childStateSet, union);
					first = false;
