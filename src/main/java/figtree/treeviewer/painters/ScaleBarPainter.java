 ...
 */
public class ScaleBarPainter extends LabelPainter<TreePane> implements
		ScalePainter {
	...
	public Rectangle2D calibrate(Graphics2D g2, TreePane treePane) {
		...
		yOffset = (float) (fm.getAscent() + topMargin + bottomMargin)
				+ scaleBarStroke.getLineWidth();
	...
	public void paint(Graphics2D g2, TreePane treePane,
		...
		case CENTER:
			xOffset = (float) (bounds.getX() + (bounds.getWidth() - rect
					.getWidth()) / 2.0);
		...
		g2.draw(new Line2D.Double(x1, bounds.getY() + topMargin, x2, bounds
				.getY() + topMargin));
