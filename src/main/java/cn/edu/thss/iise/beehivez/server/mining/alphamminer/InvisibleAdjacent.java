...
class PlaceAdjacent {
	...
	public boolean equals(Object placeAdjacent) {
		if (!(placeAdjacent instanceof PlaceAdjacent))
			return false;
		// TODO 严格来说,在这里彻底的排除掉invisible task也是不对的啊，还是应该考虑invisible
		// task的，但是invisible task需要有一个读。。。
		PlaceAdjacent pa = (PlaceAdjacent) placeAdjacent;
		HashSet<Transition> thisPreWithoutInv = removeInv(this.pre); // remove
																		// the
																		// invisible
																		// tasks....
		HashSet<Transition> thisSuccWithoutInv = removeInv(this.succ); // remove
																		// the
																		// invisible
																		// tasks....
		HashSet<Transition> paPreWithoutInv = removeInv(pa.pre); // remove the
																	// invisible
																	// tasks....
		HashSet<Transition> paSuccWithoutInv = removeInv(pa.succ); // remove the
																	// invisible
																	// tasks....
		boolean result;
		result = (thisPreWithoutInv.equals(paPreWithoutInv) || (thisPreWithoutInv
				.size() == 0 && paPreWithoutInv.size() == 0))
				&& (thisSuccWithoutInv.equals(paSuccWithoutInv) || (thisSuccWithoutInv
						.size() == 0 && paSuccWithoutInv.size() == 0));
		return result;
	}
...
public class InvisibleAdjacent {
	...
	public InvisibleAdjacent(HashSet<PlaceAdjacent> _pred,
			HashSet<PlaceAdjacent> _succ) {
		pred = _pred;
		succ = _succ;
	}
	...
	public static InvisibleAdjacent getInvisibleTask(PetriNet petriNet,
			HashSet<ModelGraphVertex> prePlaces,
			HashSet<ModelGraphVertex> postPlaces) {
