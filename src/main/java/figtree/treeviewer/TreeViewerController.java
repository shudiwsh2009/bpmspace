...
public class TreeViewerController extends AbstractController {
	...
	public enum TreeLayoutType {
		RECTILINEAR("Rectangle"), POLAR("Polar"), RADIAL("Radial");
	...
	private static Preferences PREFS = Preferences
			.userNodeForPackage(TreeViewerController.class);
	...
	public TreeViewerController(final TreeViewer treeViewer) {
		...
		final TreeLayoutType defaultLayout = TreeLayoutType.valueOf(PREFS.get(
				CONTROLLER_KEY + "." + LAYOUT_KEY, DEFAULT_LAYOUT));
		...
		rectilinearTreeLayoutController = new RectilinearTreeLayoutController(
				rectilinearTreeLayout);

		polarTreeLayout = new PolarTreeLayout();
		polarTreeLayoutController = new PolarTreeLayoutController(
				polarTreeLayout);

		radialTreeLayout = new RadialTreeLayout();
		radialTreeLayoutController = new RadialTreeLayoutController(
				radialTreeLayout);

		JPanel panel1 = new JPanel();
		panel1.setOpaque(false);
		panel1.setLayout(new BoxLayout(panel1, BoxLayout.LINE_AXIS));
		Icon rectangularTreeIcon = IconUtils.getIcon(this.getClass(),
				"images/rectangularTree.png");
		Icon polarTreeIcon = IconUtils.getIcon(this.getClass(),
				"images/polarTree.png");
		Icon radialTreeIcon = IconUtils.getIcon(this.getClass(),
				"images/radialTree.png");
		...
		rectangularTreeToggle.putClientProperty("JButton.buttonType",
				"segmentedTextured");
		rectangularTreeToggle.putClientProperty("JButton.segmentPosition",
				"first");
		polarTreeToggle
				.putClientProperty("Quaqua.Button.style", "toggleCenter");
		...
		zoomSlider = new JSlider(SwingConstants.HORIZONTAL, 0, MAX_ZOOM_SLIDER,
				0);
		...
		verticalExpansionSlider = new JSlider(SwingConstants.HORIZONTAL, 0,
				MAX_ZOOM_SLIDER, 0);
		...
		verticalExpansionSlider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent changeEvent) {
				...
				treeViewer.setVerticalExpansion(((double) value)
						/ MAX_ZOOM_SLIDER);
			}
		});

		verticalExpansionLabel = new JLabel("Expansion:");
		optionsPanel.addComponents(verticalExpansionLabel, false,
				verticalExpansionSlider, true);

		fishEyeSlider = new JSlider(SwingConstants.HORIZONTAL, 0,
				MAX_ZOOM_SLIDER, 0);
		...
		fishEyeSlider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent changeEvent) {
				final double fishEye = ((double) fishEyeSlider.getValue())
						/ MAX_ZOOM_SLIDER;
		...
		rectangularTreeToggle.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				if (rectangularTreeToggle.isSelected())
					setTreeLayout(TreeLayoutType.RECTILINEAR);
			}
		});
		polarTreeToggle.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				if (polarTreeToggle.isSelected())
					setTreeLayout(TreeLayoutType.POLAR);
			}
		});
		radialTreeToggle.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				if (radialTreeToggle.isSelected())
					setTreeLayout(TreeLayoutType.RADIAL);
			}
		});

		// Set some InputMaps and ActionMaps for key strokes. The ActionMaps are
		// set in setExpansion()
		// because they differ by whether vertical expansion is allowed for the
		// current layout.
		// The key strokes could be obtained from preferences and set in a
		...
		optionsPanel.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke("meta alt EQUALS"),
				"increaseSecondaryZoom");
		optionsPanel.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke("meta alt MINUS"),
				"decreaseSecondaryZoom");
	...
	public void setSettings(Map<String, Object> settings) {
		String treeLayoutName = (String) settings.get(CONTROLLER_KEY + "."
				+ LAYOUT_KEY);
		...
		zoomSlider.setValue((Integer) settings.get(CONTROLLER_KEY + "."
				+ ZOOM_KEY));
		verticalExpansionSlider.setValue((Integer) settings.get(CONTROLLER_KEY
				+ "." + EXPANSION_KEY));

		// These controllers are internal to TreeViewerController so settings
		// must be done here
	...
	public void getSettings(Map<String, Object> settings) {
		if (rectangularTreeToggle.isSelected()) {
			settings.put(CONTROLLER_KEY + "." + LAYOUT_KEY,
					TreeLayoutType.RECTILINEAR.name());
		} else if (polarTreeToggle.isSelected()) {
			settings.put(CONTROLLER_KEY + "." + LAYOUT_KEY,
					TreeLayoutType.POLAR.name());
		} else if (radialTreeToggle.isSelected()) {
			settings.put(CONTROLLER_KEY + "." + LAYOUT_KEY,
					TreeLayoutType.RADIAL.name());
		}
		settings.put(CONTROLLER_KEY + "." + ZOOM_KEY, zoomSlider.getValue());
		settings.put(CONTROLLER_KEY + "." + EXPANSION_KEY,
				verticalExpansionSlider.getValue());
	...
	private void setTreeLayout(TreeLayoutType layoutType) {
		...
		case RECTILINEAR:
			...
			layoutPanel.add(rectilinearTreeLayoutController.getPanel(),
					BorderLayout.CENTER);
		...
		case POLAR:
			...
			fishEyeSlider.setEnabled(true);
			layoutPanel.removeAll();
			layoutPanel.add(polarTreeLayoutController.getPanel(),
					BorderLayout.CENTER);
			fireControllerChanged();
			break;
		case RADIAL:
			...
			layoutPanel.removeAll();
			layoutPanel.add(radialTreeLayoutController.getPanel(),
					BorderLayout.CENTER);
			fireControllerChanged();
			break;
		default:
	...
	private void setExpansion() {
		if (treeViewer.verticalExpansionAllowed()) {
			...
			optionsPanel.getActionMap().put("increasePrimaryZoom",
					increaseVerticalExpansionAction);
			optionsPanel.getActionMap().put("decreasePrimaryZoom",
					decreaseVerticalExpansionAction);
			optionsPanel.getActionMap().put("increaseSecondaryZoom",
					increaseZoomAction);
			optionsPanel.getActionMap().put("decreaseSecondaryZoom",
					decreaseZoomAction);
		} else {
	...
	private Action increaseVerticalExpansionAction = new AbstractAction(
			"Expand Vertically") {
		public void actionPerformed(ActionEvent actionEvent) {
			verticalExpansionSlider.setValue(verticalExpansionSlider.getValue()
					+ DELTA_ZOOM_SLIDER);
		}
	};

	private Action decreaseVerticalExpansionAction = new AbstractAction(
			"Unexpand Vertically") {
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (value > 0) {
				...
				// If the vertical expansion was zero then assume the user is
				// trying to un-zoom
				zoomSlider.setValue(zoomSlider.getValue() - DELTA_ZOOM_SLIDER);
			}
		}
