...
public class RectilinearTreeLayout extends AbstractTreeLayout {
	...
	public void layout(RootedTree tree, TreeLayoutCache cache) {
		...
		Point2D rootPoint = constructNode(tree, root, 0.0, getRootLength(),
				cache);
	...
	private Point2D constructNode(final RootedTree tree, final Node node,
			final double xParent, final double xPosition, TreeLayoutCache cache) {
		...
		if (!tree.isExternal(node)) {
			...
			if (collapsedAttributeName != null
				...
				for (int i = 0; i < children.size(); i++) {
					...
					Point2D childPoint = constructNode(tree, child, xPosition,
							xPosition + length, cache);
				...
				for (Node child : children) {
					...
					if (curvature == 0.0) {
						...
						if (branchColouringAttribute != null) {
							colouring = (Object[]) child
									.getAttribute(branchColouringAttribute);
						}
						if (colouring != null) {
							// If there is a colouring, then we break the path
							// up into
							// segments. This should allow us to iterate along
							// the segments
							...
							// to help this, we are going to draw the branch
							// backwards
							branchPath.moveTo(x1, y1);
							float x = x1;
							for (int i = 0; i < colouring.length - 1; i += 2) {
								// float height =
								// ((Number)colouring[i+1]).floatValue();
								// float p = (height - childHeight) /
								// (nodeHeight - childHeight);
								float interval = ((Number) colouring[i + 1])
										.floatValue();
						...
						// implement it (and it would probably be confusing
						// anyway).
					...
					Line2D branchLabelPath = new Line2D.Double(x3 - 1.0, y1,
							x3 + 1.0, y1);

					cache.branchLabelPaths.put(child, branchLabelPath);
				}

				Line2D nodeLabelPath = new Line2D.Double(nodePoint.getX(), ty,
						nodePoint.getX() + 1.0, ty);

				cache.nodeLabelPaths.put(node, nodeLabelPath);

				Line2D nodeShapePath = new Line2D.Double(nodePoint.getX(), ty,
						nodePoint.getX() - 1.0, ty);
			...
			if (alignTipLabels) {
				...
				tipLabelPath = new Line2D.Double(maxXPosition, ty,
						maxXPosition + 1.0, ty);

				Line2D calloutPath = new Line2D.Double(nodePoint.getX(), ty,
						maxXPosition, ty);

				cache.calloutPaths.put(node, calloutPath);

			} else {
				tipLabelPath = new Line2D.Double(nodePoint.getX(), ty,
						nodePoint.getX() + 1.0, ty);
	...
	private void constructNodeAreas(final RootedTree tree, final Node node,
		...
		if (!tree.isExternal(node)
				&& (collapsedAttributeName == null || node
						.getAttribute(collapsedAttributeName) == null)
				&& (cartoonAttributeName == null || node
						.getAttribute(cartoonAttributeName) == null)) {
			...
			Rectangle2D branchBounds1 = cache.getBranchPath(child1)
					.getBounds2D();
			...
			Rectangle2D branchBounds2 = cache.getBranchPath(child2)
					.getBounds2D();
			...
			final float y0 = (float) (branchBounds1.getY() + branchBounds1
					.getHeight());
			...
			if (curvature == 0.0) {
				...
				final float y2 = (float) (branchBounds2.getY() + branchBounds2
						.getHeight());
				...
				final float x1 = (float) (branchBounds1.getX() + branchBounds1
						.getWidth());
				final float y1 = (float) branchBounds1.getY();
				...
				final float x2 = (float) (branchBounds2.getX() + branchBounds2
						.getWidth());
				...
				final float y3 = (float) (branchBounds2.getY() + branchBounds2
						.getHeight());
				nodePath.lineTo((float) maxXPosition, y3);

				final float x3 = (float) (branchBounds2.getX() + branchBounds2
						.getWidth());
	...
	private Point2D constructCartoonNode(RootedTree tree, Node node,
		...
		Line2D nodeLabelPath = new Line2D.Double(nodePoint.getX(), y0,
				nodePoint.getX() + 1.0, y0);

		cache.nodeLabelPaths.put(node, nodeLabelPath);

		Line2D nodeBarPath = new Line2D.Double(nodePoint.getX(), y0,
				nodePoint.getX() - 1.0, y0);
	...
	private void constructCartoonTipLabelPaths(RootedTree tree, Node node,
		...
		if (!tree.isExternal(node)) {
			...
			if (alignTipLabels) {
				...
				tipLabelPath = new Line2D.Double(maxXPosition, y0,
						maxXPosition + 1.0, y0);
	...
	private Point2D constructCollapsedNode(RootedTree tree, Node node,
		...
		Line2D nodeLabelPath = new Line2D.Double(xPosition, ty,
				xPosition + 1.0, ty);

		cache.nodeLabelPaths.put(node, nodeLabelPath);

		Line2D nodeBarPath = new Line2D.Double(xPosition, ty, xPosition - 1.0,
				ty);
		...
		if (alignTipLabels) {
			...
			Line2D calloutPath = new Line2D.Double(maxXPos, ty, maxXPosition,
					ty);
