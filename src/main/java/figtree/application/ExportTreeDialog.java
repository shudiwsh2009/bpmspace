...
public class ExportTreeDialog {
	...
	public enum Format {
		NEXUS("NEXUS"), NEWICK("Newick"), JSON("JSON");
	...
	private JCheckBox asDisplayedCheck = new JCheckBox(
			"Save as currently displayed");
	private JCheckBox allTreesCheck = new JCheckBox("Save all trees");
	private JCheckBox includeFigTreeCheck = new JCheckBox(
			"Include FigTree block (NEXUS only)");
	private JCheckBox includeAnnotationsCheck = new JCheckBox(
			"Include Annotations (NEXUS & JSON only)");
	...
	public ExportTreeDialog(JFrame frame) {
		...
		optionPane = new JOptionPane(options, JOptionPane.QUESTION_MESSAGE,
		...
		formatCombo.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				...
				includeAnnotationsCheck.setEnabled(item.equals(Format.NEXUS)
						|| item.equals(Format.JSON));
			}
		});
	}
