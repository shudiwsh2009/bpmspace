 ...
 * @version $Id: TreeAppearanceController.java,v 1.2 2007/09/10 14:52:02 rambaut
 *          Exp $
 */
public class TreeAppearanceController extends AbstractController {
	...
	public TreeAppearanceController(final TreeViewer treeViewer, String tipKey,
			final LabelPainter tipLabelPainter, String nodeKey,
			final LabelPainter nodeLabelPainter, String branchKey,
			final LabelPainter branchLabelPainter) {
		this(treeViewer, tipKey, tipLabelPainter, nodeKey, nodeLabelPainter,
				branchKey, branchLabelPainter, true);
	}

	public TreeAppearanceController(final TreeViewer treeViewer, String tipKey,
			final LabelPainter tipLabelPainter, String nodeKey,
			final LabelPainter nodeLabelPainter, String branchKey,
			final LabelPainter branchLabelPainter, boolean hideColouring) {
		...
		tipLabelPainter.setFont(new Font(defaultFontName, defaultFontStyle,
				defaultFontSize));
		tipLabelPainter.setNumberFormat(new DecimalFormat(
				defaultNumberFormatting));
		nodeLabelPainter.setFont(new Font(defaultFontName, defaultFontStyle,
				defaultFontSize));
		nodeLabelPainter.setNumberFormat(new DecimalFormat(
				defaultNumberFormatting));
		branchLabelPainter.setFont(new Font(defaultFontName, defaultFontStyle,
				defaultFontSize));
		branchLabelPainter.setNumberFormat(new DecimalFormat(
				defaultNumberFormatting));
			...
			branchColourAttributeCombo.addItemListener(new ItemListener() {
				public void itemStateChanged(ItemEvent itemEvent) {
					if (branchColourAttributeCombo.getSelectedIndex() == 0) {
						...
						String attribute = (String) branchColourAttributeCombo
								.getSelectedItem();
						if (attribute != null && attribute.length() > 0) {
							if (attribute.endsWith("*")) {
							...
							} else if (DiscreteColourDecorator.isDiscrete(
									attribute, nodes)) {
								Decorator decorator = new HSBDiscreteColourDecorator(
										attribute, nodes);

								treeViewer.setBranchColouringDecorator(null,
										null);
								...
								Decorator decorator = new InterpolatingColourDecorator(
										...
										new Color(192, 16, 0), new Color(0, 16,
												192));
			...
			for (String name : item.getAttributeNames()) {
				if (!name.startsWith("!")) {
					...
					if (!(attr instanceof Object[])) {
						...
						if (isIndex) {
							// a colouring should finish on an index (which
							// means isIndex should be false)...
							isColouring = false;
						}
	...
	public void setSettings(Map<String, Object> settings) {
		...
		if (branchColourAttributeCombo != null) {
			branchColourAttributeCombo.setSelectedItem(settings
					.get(CONTROLLER_KEY + "." + BRANCH_COLOR_ATTRIBUTE_KEY));
		}
		branchLineWidthSpinner.setValue((Double) settings.get(CONTROLLER_KEY
				+ "." + BRANCH_LINE_WIDTH_KEY));

		fontSizeSpinner.setValue(((Number) settings.get(tipKey + "."
				+ FONT_SIZE_KEY)).doubleValue());
		digitsSpinner.setValue((Integer) settings.get(tipKey + "."
	...
	public void getSettings(Map<String, Object> settings) {
		...
		settings.put(tipKey + "." + SIGNIFICANT_DIGITS_KEY,
