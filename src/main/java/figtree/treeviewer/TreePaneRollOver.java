 ...
 */
public class TreePaneRollOver extends StatusProvider.Helper implements
		MouseMotionListener {
		...
		if (tree != null) {
			...
			if (node != null) {
				...
				if (!tree.isExternal(node)) {
					...
					sb.append("Tip: \"").append(tree.getTaxon(node).toString())
							.append("\"");
				}
				sb.append(" [height = ").append(
						formatter.getFormattedValue(tree.getHeight(node)));
				sb.append(", length = ").append(
						formatter.getFormattedValue(tree.getLength(node)));
				sb.append("]");
