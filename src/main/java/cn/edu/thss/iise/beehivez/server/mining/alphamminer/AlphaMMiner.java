...
public class AlphaMMiner extends LogRelationBasedAlgorithm {
	...
	public MiningResult mine(LogReader log, LogRelations relations,
		...
		MiningResult miningResult = alphapp.mineAlphPPInfo(log, alphaPPData,
				progress);
		Date d3 = new Date();
	...
	/*
	 * @Override public MiningResult mine(LogReader log, LogRelations relations,
	 * Progress progress) { ModifiedAlphaPPProcessMiner alphapp = new
	 * ModifiedAlphaPPProcessMiner(); ModifiedAlphaSharpProcessMiner alphasharp
	 * = new ModifiedAlphaSharpProcessMiner();
	 * 
	 * //0.discover the L1L discoverL1L(log, progress); //0.3 computeL_W();
	 * //0.5 elminate the l1l eliminateL1L(); //1.mine basic relations
	 * mineBasicRelations(log, progress);
	 * 
	 * //2.mine the special relations LogEvents leEvents =
	 * relations.getLogEvents(); alphasharp.mineSpecialRelation(log,
	 * alphaPPData, progress, leEvents); //alpha sharp mine the mendacious
	 * relations MiningResult miningResult = alphapp.mineMiscInfo(log,
	 * alphaPPData,progress);
	 * 
	 * 
	 * //3.mine the alpha++ relation and construct the petrinet.
	 * 
	 * //alphapp.mineSpecialRelation(log, rmRelation, progress); (construct the
	 * process mining) //alphasharp.mineSpecialRelation(log, rmRelation,
	 * progress); //5.construct the process model (withL1L)
	 * 
	 * return miningResult; }
	 */
	private void computeL_W() {
		// TODO Auto-generated method stub
		alphaPPData.lpL_W = new L1LPlaces(alphaPPData.rmRelation,
				alphaPPData.alL1L);
		alphaPPData.lpL_W.compute();
	}

	/**
	 * discover the l1l of the logs, which are stored in the alL1L/ and also the
	 * al_log are discovered
	 * 
	 * @param log
	 * @param progress
	 */
		...
		for (int i = 0; i < np; i++) {
			...
			for (int j = 0; j < atel.size(); j++) {
				try {
					...
					if (task.equals(last1Task)
							&& !alphaPPData.alL1L.contains(task)) {
	...
	/**
	 * Mine the basic relations between tasks. basic relations are a>b, aba,->,
	 * || (these 4 relations are common in both alpha# and alpha++; NOTE: NO
	 * consider #?# = not-> and not <-..)
	 */
			...
			for (int j = 0; j < atel.size(); j++) {
				try {
					...
					if (!last1Task.equals("") && !last1Task.equals(task)) {
						// > relation
						alphaPPData.rmRelation.addRelation(last1Task, task,
								Relation.BEFORE);
						// before task
					...
					if (last2Task.equals(task) && !task.equals(last1Task)) {
						alphaPPData.rmRelation.addRelation(last2Task,
								last1Task, Relation.SHORTLOOP);
					}
					// go ahead
					last2Task = last1Task;
					last1Task = task;
