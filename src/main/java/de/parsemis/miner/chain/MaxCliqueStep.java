...
public class MaxCliqueStep<NodeType, EdgeType> extends
	 ...
	 * Fragment<NodeType, EdgeType> findMaxClique2(
	 * Collection<Embedding<NodeType, EdgeType>> embeddings,
	 ...
	 * Integer>(); Embedding<NodeType, EdgeType>[] emb = embeddings .toArray(new
	 * Embedding[embeddings.size()]); int[] nodes = new int[emb.length]; for
	 * (int i = 0; i < emb.length; ++i) nodes[i] = c.addNodeIndex(emb[i]); for
	 * (int i = 0; i < emb.length; ++i) for (int j = i + 1; j < emb.length; ++j)
	 * { if (!emb[i].overlaps(emb[j], ignore)) { c.addEdgeIndex(nodes[i],
	 * nodes[j], 0, Edge.UNDIRECTED); } } // calculate the maximum clique final
	 * BitSet clique = MaxClique.calculateHP(c, null); // return the found
	 * embeddings for (int node = clique.nextSetBit(0); node >= 0; node = clique
	 * .nextSetBit(node + 1)) ret.add(c.getNodeLabel(node)); return ret; }
	 */
