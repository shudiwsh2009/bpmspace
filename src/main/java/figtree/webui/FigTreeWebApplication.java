...
public class FigTreeWebApplication extends WApplication {
	...
	private void createControls(WVBoxLayout layout) {
		...
		box.activated().addListener(this, new Listener<Integer>() {
			...
			public void trigger(Integer choice) {
				switch (choice) {
				case 0:
					treeWidget.getTreePane().setTreeLayout(
							new RectilinearTreeLayout());
					break;
				case 1:
					treeWidget.getTreePane().setTreeLayout(
							new RadialTreeLayout());
					break;
				case 2:
					treeWidget.getTreePane().setTreeLayout(
							new PolarTreeLayout());
		...
		// @Override
		// public void trigger(Double value) {
		// tipPainter.setFont(new Font("sansserif", Font.PLAIN,
		// value.intValue()));
		// }
		// });
		spinBox.valueChanged().trigger(spinBox.getValue());
		...
		midPointCheck.changed().addListener(this, new Signal.Listener() {
			...
			public void trigger() {
				if (midPointCheck.isChecked()) {
					...
					treeWidget.getTreePane().setRootingType(
							TreePane.RootingType.MID_POINT);
				} else {
					treeWidget.getTreePane().setRootingOn(false);
					treeWidget.getTreePane().setRootingType(
							TreePane.RootingType.USER_ROOTING);
				}
			}
		});
		...
		orderCombo.changed().addListener(this, new Signal.Listener() {
			...
			public void trigger() {
				if (orderCombo.getCurrentIndex() == 0) {
					...
					treeWidget.getTreePane().setBranchOrdering(
							SortedRootedTree.BranchOrdering.values()[orderCombo
									.getCurrentIndex() - 1]);
				}
			}
		});
		...
		transformCombo.addItem(TransformedRootedTree.Transform.CLADOGRAM
				.toString());
		transformCombo.addItem(TransformedRootedTree.Transform.PROPORTIONAL
				.toString());
		transformCombo.addItem(TransformedRootedTree.Transform.EQUAL_LENGTHS
				.toString());
			...
			public void trigger() {
				if (transformCombo.getCurrentIndex() == 0) {
					...
					treeWidget
							.getTreePane()
							.setBranchTransform(
									TransformedRootedTree.Transform.values()[transformCombo
											.getCurrentIndex() - 1]);
				}
			}
		});
	...
	private WComboBox addLabelCombo(WVBoxLayout column,
			final SimpleLabelPainter labelPainter, String title) {
		...
		displayAttributeCombo.changed().addListener(this,
				new Signal.Listener() {
					@Override
					public void trigger() {
