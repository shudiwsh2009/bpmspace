...
public class ContinuousScale {
	...
	public ContinuousScale(String settings) {
		...
		if (parts.length != 4) {
			throw new IllegalArgumentException(
					"ContinousScale settings string not in correct format");
		}
	...
	public ContinuousScale(boolean normalize, double lowerRange,
			double upperRange, boolean logarithm) {
	...
	public void setAttributes(String attributeName,
		...
		if (!isNumber) {
			throw new NumberFormatException(
					"One or more values for this attribute are not numbers");
		}

		if (normalize && minValue < 0 && maxValue > 0) {
			// if normalizing, and some are negative, assume we are normalizing
			// with 0 at 0.5
			minValue = -maxValue;
		}
		if (logarithm) {
			if (minValue <= 0.0) {
				throw new NumberFormatException(
						"One or more values for this attribute are negative or zero under a log scale");
			}
