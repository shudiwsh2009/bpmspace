...
public class DAGmFragment<NodeType, EdgeType> extends
	...
	private void computePartitions() {
		...
		for (int i = 0; i < nodeCount; i++) {
			final StartPartition tmp = new StartPartition(
					hpFragment.getInDegree(i), hpFragment.getOutDegree(i),
					hpFragment.getNodeLabel(i));
	 ...
	 * 
	 * @see
	 * de.parsemis.miner.Fragment#embeddingToFragmentEdge(de.parsemis.graph.
	 * Embedding, de.parsemis.graph.Edge)
	 */
	 ...
	 * 
	 * @see
	 * de.parsemis.miner.Fragment#embeddingToFragmentNode(de.parsemis.graph.
	 * Embedding, de.parsemis.graph.Node)
	 */
	 ...
	 * 
	 * @see
	 * de.parsemis.miner.Fragment#fragmentToEmbeddingEdge(de.parsemis.graph.
	 * Embedding, de.parsemis.graph.Edge)
	 */
	 ...
	 * 
	 * @see
	 * de.parsemis.miner.Fragment#fragmentToEmbeddingNode(de.parsemis.graph.
	 * Embedding, de.parsemis.graph.Node)
	 */
		...
		return new Iterator<DataBaseGraph<NodeType, EdgeType>>() {
			...
			public boolean hasNext() {
				if (next >= 0) {
					 ...
					 * embeddings.size() + " embeddings:"); for
					 * (Embedding<NodeType,EdgeType> actEmb : embeddings) {
					 * System
					 * .out.println(actEmb.getDataBaseGraph().toGraph().getName
					 * ()); }
					 */
					return false;
				}
	...
	private final FrequentedCollection<HPEmbedding<NodeType, EdgeType>> mc() {
		if (mc == null) {
			final LocalEnvironment<NodeType, EdgeType> env = LocalEnvironment
					.env(this);
			mc = new FrequentedArrayList<HPEmbedding<NodeType, EdgeType>>(
					env.newFrequency());
