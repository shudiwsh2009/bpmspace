...
import cn.edu.thss.iise.beehivez.server.index.petrinetindex.pathindex.GenericInvertedIndex;
...
public class RltMatrixIndexNo1 extends PetriNetIndex implements RltConstants {

	private static final String indexDirectory = "processrepository/index/RltMatrixNo1";
	private static final String indexName = "RltMatrixNo1";
	private static GenericInvertedIndex rltMatrixIndex = new GenericInvertedIndex(
			indexDirectory, indexName);
	private static final String rltMatirxDir = indexDirectory + "/Matrix";
	...
	public boolean create() {
		try {
			...
			ResultSet rs = dm.executeSelectSQL("delete from rltmatrix", 0,
					Integer.MAX_VALUE, dm.getFetchSize());
			if (rs != null) {
				rs = null;
			}
			rs = dm.executeSelectSQL("select process_id from petrinet", 0,
					Integer.MAX_VALUE, dm.getFetchSize());
			...
			while (it.hasNext()) {
				...
				rs = dm.executeSelectSQL(
						"select pnml from petrinet where process_id = "
								+ cur_pid, 0, Integer.MAX_VALUE,
						dm.getFetchSize());
				if (rs.next()) {
	...
	public TreeSet<ProcessQueryResult> getProcessModels(Object o,
		...
		while (it1.hasNext()) {
			ret.add(new ProcessQueryResult(((Long) it1.next()).longValue(), 1));
		}
	...
	private boolean isMatch(SimpleNode syntaxTree, BasicRltMatrix matrix) {
		...
		case ParserTreeConstants.JJTOP:
			...
			return matrix.isRltContain(leftActivity.jjtGetValue().toString(),
					rightActivity.jjtGetValue().toString(),
					Byte.parseByte(syntaxTree.jjtGetValue().toString()));
		}
		return true;
	}
